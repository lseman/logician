...
//!
//! Most items here are dead code when not wired into `pi-natives` — they're
//! kept as a safety net and exercised by unit tests.
#![allow(dead_code)]
...
fn panic_disposition() -> PanicDisposition {
	// Upstream also treats a panic while inside `pi_shell`'s own panic scope as
	// recoverable; that crate isn't part of this fork, so only the blocking-task
	// scope applies here.
