...
use crate::{MinimizerCtx, MinimizerOutput, primitives};
...
fn filter_logs(input: &str) -> String {
	...
	crate::filters::system::compact_log_lines(
		&without_empty_runs,
		120,
		80,
		crate::filters::system::normalize_log_line,
	)
}

fn filter_docker_logs(input: &str) -> String {
	let without_empty_runs = drop_repeated_blank_lines(input);
	crate::filters::system::compact_log_lines(&without_empty_runs, 120, 80, log_dedup_key)
}
...
fn log_dedup_key(line: &str) -> String {
	if let Some((service, message)) = line.split_once('|') {
		...
		if is_compose_log_service(service) {
			...
			let normalized = crate::filters::system::normalize_log_line(message);
			return format!("{service}|{normalized}");
		}
	}
	crate::filters::system::normalize_log_line(line)
}
