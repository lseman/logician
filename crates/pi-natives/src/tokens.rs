...
pub fn count_tokens(
	...
	match input {
		...
		Either::B(array) => {
			...
			// (Upstream also gates this on an embedded shell's own
			// rayon_global_pool_available() flag; this fork has no such shell,
			// and rayon's global pool initializes lazily on first use regardless.)
			const PARALLEL_BATCH_MIN: usize = 16;
			Ok(if spans.len() >= PARALLEL_BATCH_MIN {
				spans
					.par_iter()
