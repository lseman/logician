...
test("a brand-new session resolves to the .auto layout", () => {
	...
	assert.equal(
		...
		path.join(dir, AUTO_DIR, "log.jsonl"),
	);
	assert.equal(
		sessionFilePath(dir, "prompt"),
		path.join(dir, AUTO_DIR, "prompt.md"),
	);
	assert.equal(
		sessionFilePath(dir, "config"),
		path.join(dir, AUTO_DIR, "config.json"),
	);
});
...
test("current sessions do not inherit stale legacy peer files", () => {
	...
	fs.writeFileSync(
		path.join(dir, "autoresearch.checks.sh"),
		"#!/usr/bin/env bash\n",
	);
	...
	assert.equal(
		sessionFilePath(dir, "checks"),
		path.join(dir, AUTO_DIR, "checks.sh"),
	);
});
...
test("hook scripts prefer .auto/hooks and fall back to the legacy directory", () => {
	const dir = freshDir();
	assert.equal(
		hookScriptPath(dir, "before"),
		path.join(dir, AUTO_DIR, "hooks", "before.sh"),
	);
...
test("current sessions ignore stale legacy hooks", () => {
	...
	assert.equal(
		hookScriptPath(dir, "after"),
		path.join(dir, AUTO_DIR, "hooks", "after.sh"),
	);
});
