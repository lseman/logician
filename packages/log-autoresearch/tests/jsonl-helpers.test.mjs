...
test("reconstructJsonlState ignores hooks and preserves run segments", () => {
	...
	assert.deepEqual(
		state.results.map(result => result.metric),
		[10, 7],
	);
	assert.deepEqual(
		state.results.map(result => result.segment),
		[0, 1],
	);
...
test("hook observability does not create jsonl before config exists", () => {
	...
	try {
		assert.equal(
			appendHookLogEntryIfConfigured(jsonlPath, "before", result),
			false,
		);
		...
		assert.equal(
			appendHookLogEntryIfConfigured(jsonlPath, "after", result),
			false,
		);
		assert.equal(
