...
function directionFrom(
	value: unknown,
): ReconstructedJsonlState["bestDirection"] {
...
export function isAutoresearchRunEntry(
	entry: unknown,
): entry is AutoresearchRunEntry {
	return isObjectRecord(entry) && typeof entry.run === "number";
}

function firstConfigEntry(
	jsonlContent: string,
): AutoresearchConfigEntry | null {
