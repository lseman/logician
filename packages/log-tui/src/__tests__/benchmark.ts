...
//
// Run:  npx tsx packages/log-tui/src/__tests__/benchmark.ts [--json] [--size small|medium|large|xlarge]
...
const sizeArg =
	(args.find(a => a.startsWith("--size=")) || "--size=medium")?.split("=")[1] ||
	"medium";
const iterations = 200; // frames per scenario

const SIZE_CONFIG: Record<
	string,
	{ turns: number; chunkSize: number; label: string }
> = {
	small: {
		turns: 5,
		chunkSize: 200,
		label: "Small (5 turns, ~200 chars each)",
	},
	medium: {
		turns: 20,
		chunkSize: 400,
		label: "Medium (20 turns, ~400 chars each)",
	},
	large: {
		turns: 50,
		chunkSize: 600,
		label: "Large (50 turns, ~600 chars each)",
	},
	xlarge: {
		turns: 100,
		chunkSize: 800,
		label: "XLarge (100 turns, ~800 chars each)",
	},
};
...
function benchScrollDiff(
	...
	const { ms } = measure(() => {
		const lines = transcript.render(width);
		const _viewportHeight = Math.max(1, scrollView.viewportHeight);
...
function benchFullFrame(
	...
	transcript.invalidate();
	const lines = transcript.render(width);
	const _layoutEnd = performance.now();

	// 2. Cell-level diff against previous frame
	let _changedCells = 0;
	let _cursorMoves = 0;
	const _renderWidth = width - 1;
	...
	for (let row = 0; row < termHeight; row++) {
		...
		if (rowChanges > 0) {
			_changedCells += rowChanges;
			_cursorMoves++;
		}
	}

	const _diffEnd = performance.now();

	// 3. Cursor positioning (simulated)
	_cursorMoves++; // cursor move to input bar
...
async function main() {
	...
	// ── Scenario 1: Cold Render ──────────────────────────────────────────────
	{
		const transcript = new TranscriptDisplay({
			thinkingMode: "collapsed",
			maxTurns: 40,
			maxRenderedLines: 400,
		});
	...
	// ── Scenario 4: Typing (input bar) ───────────────────────────────────────
	{
		...
		results.push({
			scenario: "typing",
			description:
				"Input bar typing — transcript cached, only input bar changes",
			iterations,
			metrics: {
				avgMs: Math.round(percentile(times, 50) * 100) / 100,
				p50Ms: Math.round(percentile(times, 50) * 100) / 100,
	...
	// ── Scenario 5: Scroll diff ──────────────────────────────────────────────
	{
		...
		results.push({
			...
			description:
				"Scrolling — partial diff at scroll boundaries, content cached",
			iterations,
			metrics: {
				avgMs: Math.round(percentile(times, 50) * 100) / 100,
				p50Ms: Math.round(percentile(times, 50) * 100) / 100,
		...
		const consistency = {
			...
			stdDevMs:
				Math.round(
					Math.sqrt(
						frameTimes.reduce(
							(s, t) => s + (t - percentile(frameTimes, 50)) ** 2,
							0,
						) / frameTimes.length,
					) * 100,
				) / 100,
			maxJitter:
				Math.round(
					(percentile(frameTimes, 99) - percentile(frameTimes, 50)) * 100,
				) / 100,
		};
		results.push({
			scenario: "frame_consistency",
			description:
				"Frame timing distribution over 100 frames (mixed idle/streaming)",
			iterations: 100,
			metrics: {
	...
	if (jsonMode) {
		...
		console.log(`--- Summary ---`);
		const hotMs =
			results.find(r => r.scenario === "hot_render")?.metrics.avgMs ?? 0;
		const streamMs =
			results.find(r => r.scenario === "streaming")?.metrics.avgMs ?? 0;
		const fullMs =
		...
		console.log(
			`  Streaming (1 new line/frame): ${streamMs.toFixed(2)}ms — target: <5ms`,
		);
		console.log(
