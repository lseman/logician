...
void test("file mention popup fuzzy-matches by basename and renders count", () => {
	...
	popup.setFiles([
		"packages/log-tui/src/app/tui.ts",
		"packages/log-tui/src/input/input-bar.ts",
	...
	assert.equal(popup.currentFile(), "packages/log-tui/src/app/tui.ts");
});
...
void test("input bar splices the accepted mention path into the active token", () => {
	...
	bar.insertMention("packages/log-tui/src/app/tui.ts");
	assert.equal(bar.valueText, "check @packages/log-tui/src/app/tui.ts ");
});
