...
function _boxLine(left: string, right: string, width: number): string {
...
function _renderSectionDivider(title: string, innerWidth: number): string {
	const pad = 1;
...
function _findActiveIndex<T>(items: T[], active: (item: T) => boolean): number {
