 ...
 */
export function buildAutoresearchCompactionSummary(
	paths: AutoresearchSummaryPaths,
): string {
...
function baselineAndBestLines(
	...
	if (!baseline) return [];
	const lines = [
		`Baseline (#${baseline.run}): ${formatMetricWithUnit(baseline.metric, unit)}`,
	];
...
function recentRunsSection(
	...
	const lines = runs.map(run =>
		formatRunLine(run, baselineFor(run, state.results)),
	);
	return [
