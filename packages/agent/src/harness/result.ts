// ── Harness Result / TaggedError ────────────────────────────────────────────
// Harness-level result and tagged-error pattern, distinct from core/result.ts
// (which models filesystem/exec/transport failures). This one backs harness
// operation outcomes (LaneBusy, NothingToCompact, etc.) that carry a stable
// `_tag` for exhaustive matching. Ported from pi coding agent's harness/result.ts.
	...
	class TaggedErrorClass extends Error {
		...
		toJSON(): { _tag: Tag; message: string } & Record<string, unknown> {
			...
			for (const key of Object.keys(this)) {
				if (key !== "_tag")
					payload[key] = (this as unknown as Record<string, unknown>)[key];
			}
...
export function matchError<TError extends TaggedErrorValue<string>, TValue>(
	...
	const matcher = (
		matchers as unknown as Record<string, (value: TError) => TValue>
	)[error._tag];
