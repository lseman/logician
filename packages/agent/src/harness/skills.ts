// ── Skills loading ────────────────────────────────────────────────────────
// Loads SKILL.md (and implicit .md) files from disk into the Skill vocabulary
// declared in harness/types.ts: recursive directory traversal, .gitignore-
// style exclusion, frontmatter parsing, name/description validation, and
// system-prompt formatting. Ported from pi coding agent's harness/skills.ts.

import ignore from "ignore";
import type { ExecutionEnv, FileInfo } from "../env/execution-env.ts";
...
/** Format the model-visible skill list block for a system prompt (skills with disableModelInvocation excluded). */
export function formatSkillsForSystemPrompt(skills: Skill[]): string {
	return skills
		.filter(skill => !skill.disableModelInvocation)
		.map(skill => formatSkillInvocation(skill))
		.join("\n\n");
}
 ...
 *
 * Source values are preserved exactly and attached to every loaded skill and diagnostic. This module does not
