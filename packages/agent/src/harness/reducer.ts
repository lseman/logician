// ── Lane state reducer ───────────────────────────────────────────────────
// Purely reconstructs one lane's orchestration state from a bounded slice of
// its durable record log — no I/O, no mutation. This is the crash-recovery
// core: replaying operation_started/step_attempt/tool_started/queue_* records
// against their entries tells the harness exactly where an interrupted run
// left off. Ported from pi coding agent's harness/reducer.ts.
...
import { isDeepEqual } from "../core/deep-equal.ts";
import type {
	Entry,
	LaneRecord,
...
function matchesProvisionedEntry(
	...
	return isDeepEqual(payload, target);
}
...
function validateAttemptSequence(
	...
	const expectedAttempt =
		continuesSeries && previousRecord ? previousRecord.attempt + 1 : 1;
...
function validateDeferredHandles(entries: Iterable<Entry>): void {
	for (const entry of entries) {
		if (
			...
			(entry.message.stopReason as string) === "deferred" &&
			!(entry.message as unknown as { deferred?: unknown }).deferred
		) {
			corrupt(
				"invalid_deferred_handle",
	...
	for (const record of records) {
		...
		switch (record.type) {
			...
			case "tool_started":
				...
				break;
			case "queue_enqueued": {
				const abortSeq = abortedAt.get(record.runId ?? "");
				if (
...
function deriveToolBatch(
	...
	const calls = toolCalls.map((toolCall, toolIndex) => {
		...
		return {
			...
			...(result?.type === "message" &&
			(result as { terminate?: boolean }).terminate === true
				? { terminate: true }
				: {}),
		};
	});
...
export function reduceLaneState(
	...
	const pendingFollowUp = aborting
		...
		: pendingQueueRecords
				.filter(
					record => record.queue === "followUp" && record.runId === started.id,
				)
				.map(record => clone(record.target));
	...
	const deferred =
		...
		(newestOwnEntry.message.stopReason as string) === "deferred" &&
		(newestOwnEntry.message as unknown as { deferred?: DeferredHandle })
			.deferred
			? clone(
					(newestOwnEntry.message as unknown as { deferred: DeferredHandle })
						.deferred,
	...
	if (
		...
		const producedByDeferredFetch =
			...
			(previousOwnEntry?.type === "message" &&
				...
				(previousOwnEntry.message.stopReason as string) === "deferred");
