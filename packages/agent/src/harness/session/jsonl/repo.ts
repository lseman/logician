// ── JSONL session repo ────────────────────────────────────────────────────
// Owns the on-disk directory layout (cwd-encoded session directories,
// timestamped filenames), session creation/listing/fork/delete, and guards
// same-process create races since the async existence check alone can't
// prevent two concurrent calls from both claiming the same {cwd, id}.
// Ported from pi coding agent's harness/session/jsonl/repo.ts.

import { uuidv7 } from "../../../core/uuid.ts";
import { assertJsonSerializable, Session } from "../session.ts";
import type { ForkOptions, SessionRepo } from "../types.ts";
...
async function jsonlSessionsRoot(
...
async function jsonlSessionDirectories(
	options: JsonlSessionRepoOptions,
	cwd?: string,
): Promise<string[]> {
	const sessionsRoot = await jsonlSessionsRoot(options);
	if (cwd !== undefined) {
		const resolvedCwd = fileResult(
			await options.fs.absolutePath(cwd),
			`Failed to resolve session cwd ${cwd}`,
		);
		const directory = await jsonlSessionDirectory(
			options.fs,
			sessionsRoot,
			resolvedCwd,
		);
		return fileResult(
			await options.fs.exists(directory),
			`Failed to check sessions directory ${directory}`,
		)
			? [directory]
			: [];
	}
	if (
		!fileResult(
			await options.fs.exists(sessionsRoot),
			`Failed to check sessions directory ${sessionsRoot}`,
		)
	)
		return [];
	return fileResult(
		await options.fs.listDir(sessionsRoot),
		`Failed to list sessions directory ${sessionsRoot}`,
	)
...
export async function listJsonlSessionMetadata(
	...
	for (const directory of await jsonlSessionDirectories(options, query.cwd)) {
		...
		for (const file of files) {
			...
			metadata.push(
				metadataFromHeader(headerResult.value, file.path, file.mtimeMs),
			);
		}
	}
...
export async function loadJsonlSessionStorage(
	...
	if (
		!fileResult(
			await options.fs.exists(metadata.path),
		...
		>
{
	...
	async create(
		...
		return this.claimCreateDestination(destination, async () => {
			...
			return new Session(
				await JsonlSessionStorage.create(this.fs, path, header),
			);
		});
	}
	...
	private async loadStorage(
		...
		return loadJsonlSessionStorage(
			{ fs: this.fs, sessionsRoot: this.sessionsRootInput },
			metadata,
		);
	}
	...
	private async prepareCreate(
		...
		fileResult(
			await this.fs.createDir(sessionDirectory, { recursive: true }),
	...
	private async listDirect(
		...
		return listJsonlSessionMetadata(
			{ fs: this.fs, sessionsRoot: this.sessionsRootInput },
			options,
		);
	}
	...
	private async sessionIdExists(id: string, cwd: string): Promise<boolean> {
		...
		if (
			!fileResult(
				await this.fs.exists(directory),
				`Failed to check sessions directory ${directory}`,
			)
		)
			return false;
