// ── JSONL-backed durable session storage ─────────────────────────────────
// Append-only log per session: a header line followed by one JSON line per
// mutation. Writes are serialized through a `tail` promise chain so
// concurrent appendEntry/appendRecord/createLane calls can't interleave.
// Load replays every line through the strict codec and, if the last line is
// an unparseable partial write (a crash mid-append — a "torn tail"),
// atomically republishes the valid prefix rather than treating it as
// corruption. Ported from pi coding agent's harness/session/jsonl/storage.ts.
...
async function publishFileAtomically(
	...
	try {
		...
		fileResult(
			await fs.renameFile(tempPath, destinationPath),
			`Failed to publish staged file ${destinationPath}`,
		);
	} catch (error) {
	...
	implements SessionStorage<JsonlSessionMetadata>
{
	...
	static async create(
		...
		fileResult(
			await fs.writeFile(path, encodeHeader(header)),
			`Failed to initialize session ${path}`,
		);
		const fileInfo = fileResult(
			await fs.fileInfo(path),
	...
	static async load(
		...
		if (physicalLines.length === 0 || !physicalLines[0]) {
			throw invalidFile(
				path,
				1,
				new JsonlDecodeError("schema", "is missing a header"),
			);
		}
	...
	async fork(
		...
		await publishFileAtomically(this.fs, path, async tempPath => {
			const targetStorage = await JsonlSessionStorage.create(
				this.fs,
				tempPath,
				header,
			);
	...
	appendRecord<TRecord extends LaneRecord>(
		...
		return this.enqueue(async () => {
			...
			const currentOpenOperationId = this.state.findOpenOperations(
				newRecord.lane,
				{ limit: 1 },
			)[0]?.id;
			if (
				newRecord.type === "operation_started" &&
				currentOpenOperationId !== undefined
			) {
