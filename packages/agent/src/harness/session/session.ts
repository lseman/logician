// ── Session ───────────────────────────────────────────────────────────────
// Validating wrapper around a SessionStorage backend: enforces JSON-serializable
// payloads and query-parameter sanity before delegating to storage. Ported from
// pi coding agent's harness/session/session.ts.
...
export function assertJsonSerializable(value: unknown): void {
	...
	while (stack.length > 0) {
		...
		if (candidate === undefined)
			invalidPayload("contains undefined");
	...
	implements SessionTree
{
	...
	view(lane: string): SessionTree {
		...
		return {
			...
			findEntryOnBranch: async (query = {}) =>
				(await this.queryBranchEntries(lane, query, 1))[0],
			appendMessage: message => this.appendMessageToLane(lane, message),
			appendCustomEntry: (customType, data) =>
				this.appendCustomEntryToLane(lane, customType, data),
		};
	}
	...
	private async getLeafIdForLane(lane: string): Promise<string | null> {
		const pointer = (await this.getLanes()).find(
	...
	private async queryEntries(
		...
		return this.storage.findEntries(
			resultLimit === query.limit ? query : { ...query, limit: resultLimit },
		);
	}
	...
	private async appendMessageToLane(
		lane: string,
	...
	private async appendCustomEntryToLane(
		lane: string,
		customType: string,
		data?: unknown,
	): Promise<string> {
	...
	private async commitEntry<TEntry extends Entry>(
		entry: ProvisionedEntry<TEntry>,
		lane: string,
	): Promise<TEntry> {
