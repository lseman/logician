...
// Projects a path of durable session Entry objects into the AgentMessage[]
// (plus effective model/thinking-level/tools state) an agent-loop turn would
// see. Ported from pi coding agent's harness/session/context.ts.
...
export function sessionEntryToContextMessages(
	...
	if (entry.type === "custom") {
		return [
			...(options.entryProjectors?.[entry.customType]?.(
				entry,
				index,
				entries,
			) ?? []),
