// ── Session log types ────────────────────────────────────────────────────
// Durable append-only log vocabulary: Entry (transcript-visible content) and
// LaneRecord (operation lifecycle bookkeeping the reducer replays for crash
// recovery). Ported from pi coding agent's harness/session/types.ts, adapted
// to agent's own ai/ and agent/ modules.

import type { AgentMessage } from "../../agent/types.ts";
import type { StopReason, Usage } from "../../ai/types.ts";
...
export type ProvisionedEntry<TEntry extends Entry = Entry> =
	TEntry extends Entry ? Omit<TEntry, "parentId" | "seq" | "timestamp"> : never;
...
export type LaneRecord =
	...
	| WriteDeferredRecord
	| UsageRecord;
export type NewRecord<TRecord extends LaneRecord = LaneRecord> =
	TRecord extends LaneRecord ? Omit<TRecord, "seq" | "timestamp"> : never;
...
> {
	...
	): Promise<import("./session.ts").Session<TMetadata>>;
	/** Opens the session for writing and acquires any backend writer claim. */
	open(metadata: TMetadata): Promise<import("./session.ts").Session<TMetadata>>;
