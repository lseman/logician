...
// Pure in-memory reducer over durable session mutations (entry/record/lane/fact).
// No I/O — this is the shared engine both the in-memory and JSONL-backed
// SessionStorage implementations build on: apply a mutation, keep query
// indices (entries by id, records by lane, open operations, log) consistent.
// Ported from pi coding agent's harness/session/state.ts.
...
export class SessionState {
	...
	private readonly openOperationsByLane = new Map<
		string,
		Map<string, OperationStartedRecord>
	>();
		...
		switch (mutation.kind) {
			case "entry": {
				...
				if (
					...
					!this.entriesById.has(mutation.entry.parentId)
				) {
			...
			case "record": {
				...
				if (mutation.record.type === "operation_started") {
					let openOperations = this.openOperationsByLane.get(
					...
					this.openOperationsByLane
						.get(mutation.record.lane)
						?.delete(mutation.record.runId);
				}
	...
	findOpenOperations(
		...
		const openOperations = openOperationsById
			? [...openOperationsById.values()].reverse()
			: [];
		return options?.limit === undefined
			? openOperations
			: openOperations.slice(0, options.limit);
	}
