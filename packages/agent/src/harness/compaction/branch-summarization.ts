...
// Summarizes an abandoned conversation branch before navigating away from it,
// so returning later has a structured recap instead of losing the exploration
// entirely. Session-log-native: walks the durable Entry parent chain from the
// old leaf back to the common ancestor with the navigation target. Ported
// from pi coding agent's harness/compaction/branch-summarization.ts.
...
import { BranchSummaryError } from "../../core/errors.ts";
...
import type { Entry, Session } from "../session/index.ts";
import { SessionError } from "../session/types.ts";
...
export interface GenerateBranchSummaryOptions {
	...
	/** Reasoning level forwarded to the summarization request, if the model supports it. */
...
export async function collectEntriesForBranchSummary(
	...
	const oldPath = new Set(
		(await session.findEntriesOnBranch({ start: oldLeafId })).map(
			entry => entry.id,
		),
	);
