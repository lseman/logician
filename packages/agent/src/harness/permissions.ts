...
// Ported from the old @logician/agent-core's harness/tools/permissions.ts,
// adapted to this package's AgentTool/ToolCall shapes (ai/types.ts, agent/types.ts).
//
// Rule syntax (string):
...
/**
 * Bridge a PermissionManager to AgentHarness's `before_tool` hook. "ask" verdicts are
 * resolved via `onAsk` (the actual prompt UI is app-specific, e.g. TUI's approval overlay);
 * returning true from `onAsk` allows the call once, "always" additionally session-allows it.
 */
export function createPermissionHook(
	...
	return async (
		...
		const verdict = permissions.evaluate(ctx.toolCall, args, tool);
		if (verdict.decision === "allow") return undefined;
		if (verdict.decision === "deny") {
			return {
				block: true,
				reason: verdict.reason ?? "Denied by permission rules.",
			};
		}
		const answer = await onAsk(ctx.toolCall, args, verdict.reason);
		if (answer === "deny") {
			return { block: true, reason: "Denied by user." };
		}
		if (answer === "always") {
			permissions.addSessionAllow(ctx.toolCall.name, args);
		}
		return undefined;
	};
}
