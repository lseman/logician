// ── Agent core types ─────────────────────────────────────────────────────
// Ported from pi coding agent's types.ts, adapted to agent's own
// ai/ transport module instead of @earendil-works/pi-ai.
...
export type { AssistantMessageEventStream } from "../ai/types.ts";
...
/**
 * Stream function used by the agent loop. `streamSimple` (ai/openai-completions.ts) satisfies
 * this shape.
 *
 * Contract:
...
export interface AgentLoopConfig extends SimpleStreamOptions {
	 ...
	 *
	 * Useful for short-lived OAuth tokens that may expire during long-running tool execution phases.
	 *
 ...
 * ```
 */
// biome-ignore lint/suspicious/noEmptyInterface: intentionally empty — apps extend this via declaration merging (a type alias can't be augmented this way).
...
export interface AgentState {
	...
	set tools(tools: AgentTool<TSchema, unknown>[]);
	get tools(): AgentTool<TSchema, unknown>[];
...
> extends Tool<TParameters> {
	...
	label: string;
	/**
	 * True if this tool never mutates state (filesystem, network, process). Consulted by
	 * permission gating (harness/permissions.ts) to auto-allow reads regardless of mode.
	 */
	readOnly?: boolean;
	/**
