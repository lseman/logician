// Ported from pi coding agent's agent.ts, adapted to agent's own ai/ transport module.
...
export class Agent {
	...
	constructor(options: AgentOptions) {
		...
		this.steeringQueue = new PendingMessageQueue(
			runtimeOptions.steeringMode ?? "one-at-a-time",
		);
		this.followUpQueue = new PendingMessageQueue(
			runtimeOptions.followUpMode ?? "one-at-a-time",
		);
				...
				this.prepareNextTurnWithContext || this.prepareNextTurn
					? async context => {
							if (this.prepareNextTurnWithContext) {
								return await this.prepareNextTurnWithContext(
									context,
									this.signal,
								);
							}
