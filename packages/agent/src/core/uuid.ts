// ── UUIDv7 ────────────────────────────────────────────────────────────────
// Ported from pi-ai's utils/uuid.ts. Time-ordered UUIDs are used as durable
// session entry/record ids so lexicographic order matches insertion order.
...
export function uuidv7(): string {
	...
	bytes[10] = ((sequence & 0x3f) << 2) | ((random[10] ?? 0) & 0x03);
	bytes[11] = random[11] ?? 0;
	bytes[12] = random[12] ?? 0;
	bytes[13] = random[13] ?? 0;
	bytes[14] = random[14] ?? 0;
	bytes[15] = random[15] ?? 0;
