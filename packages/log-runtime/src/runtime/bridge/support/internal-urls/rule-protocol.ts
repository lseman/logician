...
interface RuleStore {
	rules: Map<string, { name: string; content: string; path: string; description: string }>;
}
...
export class RuleProtocolHandler implements ProtocolHandler {
	...
	resolve(url: InternalUrl, _context?: ResolveContext): Promise<InternalResource> {
		const store = _store;
		if (!store) {
	...
	complete(query: string, _context?: ResolveContext): Promise<UrlCompletion[]> {
		...
		const rules = Array.from(store.rules.values());
		const matches = query
			? rules.filter(r => r.name.toLowerCase().includes(query.toLowerCase()) || r.description.toLowerCase().includes(query.toLowerCase()))
			: rules;
