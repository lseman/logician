...
// ============================================================================
// Static prefix builder & Dynamic context builder (Prompt Caching SOTA)
// ============================================================================
...
- Keep changes scoped to the request. Never use destructive git operations (reset --hard, checkout --, deletions) unless explicitly asked.${webSection}`;
	...
	if (
		...
		const sortedSkills = [...providedSkills].sort((a, b) =>
			a.name.localeCompare(b.name),
		);
...
export function buildSystemPrompt(options: BuildSystemPromptOptions): string {
	const staticPrefix = buildStaticSystemPromptPrefix(options);
	const dynamicContext = buildDynamicSystemContext({ cwd: options.cwd });
