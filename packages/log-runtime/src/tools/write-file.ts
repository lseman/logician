...
import { highlightAuto } from "../shared/syntax-highlighter.ts";
import {
	DEFAULT_MAX_BYTES,
	DEFAULT_MAX_LINES,
	formatSize,
	...
	return {
		...
		execute: async (args, ctx): Promise<string | ToolResult> => {
			...
			if (!scheme) {
				...
				const archiveMatch = detectArchiveSelector(filePath, cwd, {
					requireExisting: false,
				});
				if (archiveMatch && archiveMatch.selector) {
					if (append)
						return "Error: append is not supported for archive members.";
					return withFileMutationQueue(archiveMatch.absolutePath, async () => {
						...
						if (!family)
							return `Error: unsupported archive format for ${archiveMatch.absolutePath}.`;
						try {
				...
				const sqliteMatch = detectSqliteSelector(filePath, cwd, {
					requireExisting: false,
				});
				if (sqliteMatch && sqliteMatch.selector) {
					...
					return withFileMutationQueue(sqliteMatch.absolutePath, async () => {
						ensureInsideCwd(
							ctx.cwd,
							sqliteMatch.absolutePath,
							ctx.allowedPaths,
							ctx.allowAllPaths,
						);
						const [table, rowid] = sqliteMatch.selector.split(":");
						if (!table)
							return "Error: SQLite writes require a table name, e.g. db.sqlite:table.";
						const trimmed = content.trim();
						try {
							let resultText: string;
							if (rowid === undefined) {
								resultText = insertSqliteRow(
									sqliteMatch.absolutePath,
									table,
									content,
								);
							} else if (trimmed.length === 0) {
								resultText = deleteSqliteRow(
									sqliteMatch.absolutePath,
									table,
									rowid,
								);
							} else {
								resultText = updateSqliteRow(
									sqliteMatch.absolutePath,
									table,
									rowid,
									content,
