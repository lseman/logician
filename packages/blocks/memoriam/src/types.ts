// ── Memoriam — Standalone Memory Algorithm Types ───────────────────────────────
// Persistent memory: observation→compression→memory pipeline.
// SQLite-backed with synthetic compression, consolidation, retrieval, and
// working-memory tiering. No LLM dependency required.
...
export interface RawObservation {
	...
	sessionId: string;
	timestamp: string;
	/** Arbitrary phase label; agents can use their own phase names. */
	hookType: string;
	toolName?: string;
	toolInput?: unknown;
	toolOutput?: unknown;
