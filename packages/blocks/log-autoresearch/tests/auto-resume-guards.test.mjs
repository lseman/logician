...
test("auto-resume turn ceiling is 200", () => {
	...
	assert.match(autoResumeStopReasonFor(runtime(200, [])), /200 turns/);
});
...
test("consecutive discard/crash override stops after more than 20 failures", () => {
	...
	assert.equal(
		countConsecutiveDiscardOrCrashResults(state(failuresAtLimit)),
		20,
	);
	...
	assert.equal(
		countConsecutiveDiscardOrCrashResults(state(failuresPastLimit)),
		21,
	);
	assert.match(
...
test("consecutive failure override resets on keep and ignores previous segments", () => {
	...
	assert.equal(
		countConsecutiveDiscardOrCrashResults(
			state([...failuresPastLimit, result("keep")]),
		),
		0,
	);
	assert.equal(
	...
	const previousSegmentFailures = failuresPastLimit.map(entry => ({
		...entry,
		segment: 0,
	}));
