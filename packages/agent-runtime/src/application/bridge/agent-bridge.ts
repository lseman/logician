...
import { OpenAIBackend } from "@logician/agent-core";
import { AgentHarness } from "@logician/agent-core/harness";
import type { PermissionMode } from "@logician/agent-core/permissions";
import type { AbortResult, Session } from "@logician/agent-core/runtime";
import {
...
import { RuntimeModelManager } from "../runtime/model-manager.ts";
import {
...
import { BridgeSessionManager } from "./session.ts";
...
export class AgentRuntime {
	...
	constructor(
		...
		this.toolRouter = new ToolRouter({
			...
			onToolAdded: _tool => {
				...
				this.harness?.configure({ tools: this.config.tools });
			},
			onContextChanged: () => {
	...
	private async runMessage(message: string): Promise<void> {
		...
		try {
			...
			if (repositoryContext) {
				...
				harness.configure({ systemPrompt: turnSystemPrompt });
			}
			...
			if (persistentSystemPrompt !== undefined) {
				this.harness?.configure({ systemPrompt: persistentSystemPrompt });
			}
			this.running = false;
	...
	private ensureHarness(): AgentHarness {
		if (!this.harness) {
			...
			this.harness.observe({
				queueChange: () => this.sessionManager?.emitQueueUpdate(),
				phaseChange: phase => this.sessionManager?.emitHarnessPhase(phase),
	...
	private setThinkingLevel(level: string): void {
		this.config.thinkingLevel = level as
			...
			| "high"
			| "xhigh";
		this.harness?.models.setThinkingLevel(level);
	...
	private setTemperature(temperature: number): void {
		...
		this.harness?.configure({ temperature });
	}
	...
	private applyRuntimeToggle(key: RuntimeToggleKey, enabled: boolean): void {
		...
		if (
			...
			this.harness?.configure({ [key]: enabled });
		}
