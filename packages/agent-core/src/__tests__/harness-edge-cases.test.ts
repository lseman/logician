...
import { AgentHarness, HarnessBusyError } from "../agent/harness/harness.ts";
...
void test("setTools with empty list removes all tools", async () => {
	const harness = makeHarness(new FakeBackend([() => textResponse("a")]));
	harness.setTools([]);
...
void test("setOnPhaseChange fires on turn transitions", async () => {
	...
	const harness = makeHarness(new FakeBackend([() => textResponse("a")]));
	harness.setOnPhaseChange((from, to) => {
		phases.push([from, to]);
	});
	await harness.prompt("q");
	assert.ok(
