...
): { router: ToolRouter; onContextChangedCalls: number[]; onToolAddedCalls: Tool[] } {
...
void test("setFffgrepEnabled(false) has no effect when no fff tools exist", () => {
	...
	assert.equal(onContextChangedCalls.length, 0, "onContextChanged should NOT fire");
});

void test("setFffgrepEnabled(true) has no effect when no fff tools are disabled", () => {
	const { router, onContextChangedCalls } = makeRouter({ fffgrepEnabled: false });
	const before = router.getDefaultTools().map((t) => t.name);
...
void test("setFffgrepEnabled(true) when already enabled is a no-op", () => {
	const { router, onContextChangedCalls } = makeRouter({ fffgrepEnabled: true });
