...
// Extracted from types-messages.ts to keep that file focused on messages,
// tools, and events. The barrel (types.ts) still re-exports everything.

import type { AgentMessage } from "./types-messages.ts";
...
export interface AfterProviderResponseContext {
	...
	stopReason: import("./types-messages.ts").StopReason;
	usageTokens?: number;
	iteration: number;
}
