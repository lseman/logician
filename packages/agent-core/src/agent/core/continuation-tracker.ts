...
export class ContinuationTracker {
	...
	requestContinuation(progressFingerprint: string): ContinuationDecision {
		...
		const elapsedMs = Math.max(0, current.updatedAt - (current.createdAt ?? current.updatedAt));
	...
	budgetStatus(now = Date.now()): RunBudgetStatus | undefined {
		...
		return {
			continuationsRemaining: Math.max(0, this.limits.maxRuns - state.continuationRuns),
