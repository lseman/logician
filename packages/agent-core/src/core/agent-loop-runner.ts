...
export interface TaskAwareCallbacks {
	...
	getTaskStatus: () => { status: string; summary: string; ts: number } | null | undefined;
	...
	}) => { status: RunOutcomeStatus; summary?: string; source: "structured" | "heuristic" | "runtime" };
}
...
export interface ContinuationDecision {
	...
	outcome?: { status: RunOutcomeStatus; summary?: string; source: "structured" | "heuristic" | "runtime" };
}
...
export function resolveOutcomeDefault(ctx: {
	...
	if (ctx.declared) {
		return {
			status: (ctx.declared.status === "done" ? "completed" : ctx.declared.status) as RunOutcomeStatus,
