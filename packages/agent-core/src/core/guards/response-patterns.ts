...
export function awaitsUserInput(text: string): boolean {
	...
	if (/\?[\s*_`"'"]*\s*$/.test(trimmed)) {
		const beforeQuestion = trimmed.replace(/\?[\s*_`"'"]*\s*$/, "");
		const lastSentence =
			beforeQuestion
				.split(/(?:[.!:]|\n{2,})/)
				.filter(Boolean)
		...
		if (
			...
			/\b(?:check|verify|try|approach|think|consider|maybe|perhaps)\b/i.test(
				lastSentence,
			) &&
			!/\b(?:what should|how can|i need your|please |tell me which|let me know|can you help|should i proceed|choose |confirm )/i.test(
				lastSentence,
			)
		) {
