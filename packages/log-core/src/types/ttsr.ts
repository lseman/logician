...
export interface TtsrRule {
	...
	/**
	 * Natural-language yes/no question a judge model answers about each
	 * completed in-scope output. When set, `conditions`/`astConditions` only
	 * prefilter which outputs are judged; the rule never matches mid-stream.
	 */
	question?: string;
	...
	builtin?: boolean;
	/** Agent name globs this rule applies to (e.g. ["main", "scout"]). */
	agents?: string[];
}
...
export interface TtsrSettings {
	...
	/** Ask the judge model about judged (`question`) rules. Off → those rules are inert. */
	judge: boolean;
}

// ── Judged rules ──────────────────────────────────────────────────────────────

/** One completed assistant output (reply, reasoning, or a tool call) as judged rules see it. */
export interface TtsrOutput {
	content: string;
	context: TtsrMatchContext;
	/** How warnings name the output, e.g. "reply" or "`write` call on `src/a.ts`". */
	subject: string;
}
