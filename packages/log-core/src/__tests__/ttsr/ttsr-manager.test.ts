...
import type { TtsrMatchContext } from "../../types/ttsr.ts";
...
function makeRule(
	overrides?: Partial<Parameters<TtsrManager["addRule"]>[0]>,
): Parameters<TtsrManager["addRule"]>[0] {
	return {
		name: "test-rule",
		path: "test.md",
		description: "test",
...
function mgrWithSettings(
	overrides: ConstructorParameters<typeof TtsrManager>[0],
): TtsrManager {
	return new TtsrManager(overrides);
}
...
describe("TtsrManager", () => {
	...
	it("scope:thinking matches thinking tokens", () => {
		...
		assert.equal(
			mgr.checkDelta("that is a bad idea", textCtx("thinking")).length,
			1,
		);
		assert.equal(
			mgr.checkDelta("that is a bad idea", textCtx("text")).length,
			0,
		);
	});
...
describe("TTSR scope validation", () => {
	it("accepts stream scopes and named tools, rejecting malformed scopes", async () => {
		const { isTtsrScope } = await import("../../types/ttsr.ts");
		for (const scope of ["text", "thinking", "tool", "tool:custom_tool"])
			assert.equal(isTtsrScope(scope), true);
		for (const scope of ["", "unknown", "tool:", "tool:bad name", "tool:a:b"])
			assert.equal(isTtsrScope(scope), false);
	});
});
