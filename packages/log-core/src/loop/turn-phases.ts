...
import type { AgentSettings } from "../config/agent-settings.ts";
import { resolveAgentSettings } from "../config/agent-settings.ts";
...
import type { SoftToolRequirementManager } from "../guards/soft-tool-requirement.ts";
import {
	MAX_ESCALATIONS,
	SoftToolRequirementExceededError,
} from "../guards/soft-tool-requirement.ts";
import type { TextLoopDetector } from "../guards/text-loop-detector.ts";
import type { ResolvedExecutionPolicy } from "../policy/execution-policy.ts";
import { evaluateStopPolicies } from "../policy/execution-policy.ts";
import { checkBudget } from "../policy/exit-path.ts";
import type {
	HarnessInterventionController,
	InterventionInput,
} from "../policy/intervention-controller.ts";
import type { RunBudgetController } from "../policy/run-budget.ts";
import type { AgentRunController } from "../policy/run-controller.ts";
import { createVerifiedStopPolicy } from "../policy/verified-stop-policy.ts";
import type { LLMResponse } from "../provider/backend.ts";
import type { resolveTokenEncoding } from "../provider/messages.ts";
import {
	createSystemMessage,
...
import type { ToolResultCache } from "../tools/tool-result-cache.ts";
import type { RunOutcomeStatus } from "../types/execution-policy.ts";
import type {
	AgentEventSink,
	AgentMessage,
	CompactableMessage,
...
import type { RunBudgetDecision } from "../types/run-budget.ts";
import { isToolFailureResult } from "./adaptive-mode.ts";
import type {
	RunAgentLoopConfig,
	RunAgentLoopContext,
...
import type { PrefixDivergence } from "./prefix-stability.ts";
import { processProviderResponse } from "./provider-response.ts";
import type { ProviderTurnState } from "./provider-turn.ts";
import { requestAssistantTurn } from "./provider-turn.ts";
import type { ToolBatchResult } from "./tool-batch-controller.ts";
...
export async function evaluateStopPolicyPhase(
	...
	if (decision?.action === "continue" && decision.messages.length > 0) {
		await intervene(ctx, {
			...
			message: `Stop policy ${decision.policyId} requested continuation with ${decision.messages.length} message(s).`,
			iteration: ctx.iteration,
		});
