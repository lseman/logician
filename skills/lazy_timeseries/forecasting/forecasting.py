...
__skill__ = {
    "name": "Forecasting",
    "description": "Use for fitting, comparing, and running forecast models on time-series data.",
    "workflow": [
        "Forecast only after the series is loaded and minimally validated.",
        "Compare models and cross-validate before claiming one is best.",
    ],
}
...
def stat_forecast(
    ...
    Avoid when: The user is still trying to load or clean data and has not asked for future predictions yet..
    Inputs: method (str, required): One of 'naive', 'snaive', 'moving_avg', 'ses', 'holt_winters', 'auto_arima', 'auto_ets', 'auto_theta', 'auto_ces', 'adida', 'croston', 'mstl'; periods (int, required): Forecast horizon; season_length (int, optional): Seasonal period (default 0 = auto-detect); level (list[int], optional): Prediction interval levels (default [80, 95]).
    ...
    col = (
        "value"
        if "value" in ctx.data.columns
        else (ctx.value_columns[0] if ctx.value_columns else None)
    )
    if col is None:
...
def neural_forecast(
    ...
    """Use when: Deep learning forecast using Nixtla's NeuralForecast. Supports SOTA architectures including N-BEATS, N-HiTS, PatchTST, TimesNet, TFT, iTransformer, TiDE, KAN, and more.

    Triggers: forecast the next, compare models, choose a forecast model, rolling evaluation, prediction intervals, neural forecast.
    Avoid when: The user is still trying to load or clean data and has not asked for future predictions yet..
    Inputs: method (str, required): One of 'nbeats', 'nhits', 'patchtst', 'timesnet', 'tft', 'itransformer', 'timellm', 'mlp', 'lstm', 'tide', 'kan'; periods (int, required): Forecast horizon; season_length (int, optional): Seasonal period (default 0 = auto-detect). Used to scale input_size.; level (list[int], optional): Prediction interval levels (default [80, 95]); max_steps (int, optional): Override training steps (default 0 = use model default); input_size_multiplier (int, optional): input_size = multiplier × season_length (default 0 = use 2×); extra_kwargs (dict, optional): Additional model kwargs to override defaults.
    ...
    if len(df) < 30:
        return _safe_json(
            {"status": "error", "error": "Neural models need >= 30 observations"}
        )
    ...
    try:
        ...
        mult = (
            int(input_size_multiplier)
            if input_size_multiplier
            else kwargs.get("input_size", 2)
        )
        ...
        if base_col is None:
            return _safe_json(
                {"status": "error", "error": "No forecast column in output"}
            )
...
def ensemble_forecast(
    ...
    Triggers: forecast the next, compare models, choose a forecast model, rolling evaluation, prediction intervals, neural forecast.
    Avoid when: The user is still trying to load or clean data and has not asked for future predictions yet..
    Inputs: stat_methods (list, optional): Statistical method names from stat_forecast registry; neural_methods (list, optional): Neural method names from neural_forecast registry; periods (int, required): Forecast horizon; season_length (int, optional): Seasonal period (default 0 = auto); level (list[int], optional): Prediction interval levels (default [80, 95]); cv_windows (int, optional): Number of cross-validation windows for weighting (default 3); weighting (str, optional): 'inverse_mae', 'inverse_rmse', or 'equal' (default 'inverse_mae').
    ...
    def _extract_scores(cv_df, models_list):
        ...
        for m_obj in models_list:
            ...
            matching = [
                ...
                if m_name in c and "lo" not in c and "hi" not in c
            ]
            if matching:
        ...
        try:
            ...
            if models:
                ...
                cv_df = sf.cross_validation(
                    df=df, h=h, n_windows=cv_windows, level=level
                ).reset_index()
    ...
    if weighting == "equal" or not method_scores:
        ...
        raw = {
            m: 1.0 / (scores[metric_key] + 1e-8) for m, scores in method_scores.items()
        }
        ...
        if unscored:
            avg_w = (
                np.mean(list(weights.values())) if weights else 1.0 / len(all_methods)
            )
            for m in unscored:
                weights[m] = avg_w
    ...
    resids = y[-min(50, len(y)) :] - np.mean(y[-min(50, len(y)) :])
    boots = np.array(
        [ensemble_fc + np.random.choice(resids, size=h) for _ in range(200)]
    )
...
@tool
def cross_validate(
    methods=None, neural_methods=None, horizon=0, n_windows=5, season_length=0
):
    """Use when: Rolling-origin cross-validation using Nixtla's built-in `.cross_validation()`. Returns per-method MAE, RMSE, sMAPE and a ranked leaderboard.

    Triggers: forecast the next, compare models, choose a forecast model, rolling evaluation, prediction intervals, neural forecast.
    Avoid when: The user is still trying to load or clean data and has not asked for future predictions yet..
    Inputs: methods (list, optional): Statistical method names; neural_methods (list, optional): Neural method names; horizon (int, required): Forecast horizon per window; n_windows (int, optional): Number of CV windows (default 5); season_length (int, optional): Seasonal period (default 0 = auto).
                    ...
                    res[m_name] = {
                        ...
                        "smape": float(
                            np.mean(
                                2
                                * abs_err
                                / (np.abs(actuals[mask]) + np.abs(preds[mask]) + 1e-8)
                            )
                            * 100
                        ),
...
@tool
def suggest_models():
    """Use when: Analyze data characteristics and suggest appropriate forecasting models with reasoning.
    ...
    if characteristics["has_seasonality"]:
        ...
        suggestions["reasoning"].append(
            f"Detected seasonality (period ~{s}), added seasonal models"
        )
    else:
        suggestions["stat_baselines"].extend(["auto_theta", "ses"])
        suggestions["reasoning"].append(
            "No clear seasonality, sticking with non-seasonal models"
        )

    if characteristics["intermittent"]:
        suggestions["stat_baselines"].extend(["croston", "adida"])
        suggestions["reasoning"].append(
            "Intermittent demand detected, added Croston/ADIDA"
        )

    if n >= 100:
        suggestions["neural_models"].append("nhits")
        suggestions["reasoning"].append(
            "N-HiTS: good default neural model, handles multi-scale patterns"
        )
        if n >= 200:
            suggestions["neural_models"].append("nbeats")
            suggestions["reasoning"].append(
                "N-BEATS: strong with enough data, interpretable decomposition"
            )
        if n >= 300 and characteristics["has_seasonality"]:
            suggestions["neural_models"].append("patchtst")
            suggestions["reasoning"].append(
                "PatchTST: excellent for seasonal data with sufficient length"
            )
            suggestions["neural_models"].append("timesnet")
            suggestions["reasoning"].append(
                "TimesNet: captures multi-period temporal patterns via 2D convolutions"
            )
        if n >= 500:
            suggestions["neural_models"].append("tft")
            suggestions["reasoning"].append(
                "TFT: powerful with long history, attention-based interpretability"
            )
            suggestions["neural_models"].append("itransformer")
            suggestions["reasoning"].append(
                "iTransformer: inverted attention works well on longer series"
            )
    elif n >= 50:
        suggestions["neural_models"].extend(["mlp", "nhits"])
        suggestions["reasoning"].append(
            "Limited data: MLP and N-HiTS are most data-efficient neural models"
        )
    else:
        suggestions["reasoning"].append(
            "Too few observations for neural models, stick with statistical methods"
        )
...
__tools__ = [stat_forecast, neural_forecast, ensemble_forecast, cross_validate, suggest_models]
