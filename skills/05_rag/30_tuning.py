...
@llm.tool(
    description="Inspect active RAG retrieval settings (rerank, thresholds, and index metadata)."
)
...
@llm.tool(
    description=(
        "Apply a retrieval profile (speed|balanced|quality|max_quality) to RAG runtime knobs "
        "without reindexing."
    )
)
...
@llm.tool(
    description="Benchmark RAG retrieval latency for a query over multiple runs."
)
def rag_benchmark(
    query: str,
    top_k: int = 20,
