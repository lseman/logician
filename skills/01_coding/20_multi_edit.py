...
@llm.tool(
    description=(
        "Apply multiple independent string replacements in one call, across one or more files. "
        "Each replacement is {file, old_string, new_string}. "
        "old_string must appear exactly once in its file (include 3-5 lines of context "
        "before and after to make it unambiguous). "
        "Failed hunks are reported individually without blocking the rest. "
        "Use this instead of calling edit_file_replace multiple times — it is faster and "
        "preserves the inter-hunk order."
    )
)
