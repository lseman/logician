...
@llm.tool(description="Run an arbitrary shell command and return stdout/stderr.")
def run_shell(
...
@llm.tool(
    description="Set the default virtual-environment used by run_shell and run_python."
)
def set_venv(venv_path: str) -> str:
    ...
    Side effects: Updates the module-level _coding_config dict for this session.
    """
    ...
    _coding_config["venv_path"] = str(p)
    return _safe_json(
        {
            "status": "ok",
            "venv_path": str(p),
...
@llm.tool(description="Set the default working directory for shell commands.")
def set_working_directory(path: str) -> str:
    ...
    Side effects: Updates _coding_config for this session.
    """
    p = Path(path).expanduser().resolve()
    if not p.is_dir():
        return _safe_json({"status": "error", "error": f"Not a directory: {p}"})
    _coding_config["default_cwd"] = str(p)
    return _safe_json({"status": "ok", "cwd": str(p)})


@llm.tool(description="Execute a Python code snippet and capture its output.")
...
@llm.tool(
    description="Install Python packages into the configured (or specified) virtualenv."
)
...
@llm.tool(
    description="Show the current coding session config: venv and working directory."
)
def show_coding_config() -> str:
    ...
    """
    venv = _coding_config.get("venv_path")
...
@llm.tool(
    description="Start a long-running background process (server, watcher, etc.) and keep it alive."
)
def start_background_process(
    ...
    prefix = _build_shell_prefix(venv_path or None)
    full_cmd = prefix + command if prefix else command
    resolved_cwd = _resolve_cwd(cwd or None)
...
@llm.tool(description="Read accumulated stdout from a running background process.")
...
@llm.tool(description="Terminate a named background process.")
...
@llm.tool(description="List all currently tracked background processes.")
...
@llm.tool(
    description="List installed packages in the configured or specified virtualenv."
)
def list_installed_packages(venv_path: str = "") -> str:
    ...
    r = _run_cmd(cmd, timeout=30, venv_path=venv_path or None)
    packages = []
    try:
        raw = r["stdout"].strip()
        start = raw.find("[")
...
@llm.tool(
    description="Check which Python module names are importable in the configured environment."
)
def check_imports(modules: str, venv_path: str = "") -> str:
    ...
    with tempfile.NamedTemporaryFile(
        ...
        tmp_path = tmp.name
    try:
        r = _run_cmd(f"{python} {tmp_path}", timeout=20, venv_path=None)
    finally:
        try:
            Path(tmp_path).unlink()
        except Exception:
