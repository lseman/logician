...
@llm.tool(
    description=(
        "Preview the unified diff between a file's current content and proposed new content "
        "WITHOUT writing anything. Use before apply_unified_diff to verify your changes."
    )
)
...
@llm.tool(description="Show the unified diff between two existing files.")
...
@llm.tool(
    description=(
        "Apply a unified diff string to a file in-place. "
        "The diff must be in standard git/difflib unified format (--- / +++ / @@ headers). "
        "Use diff_preview first to verify the diff before applying."
    )
)
...
@llm.tool(
    description=(
        "Apply multiple unified diffs to multiple files in one atomic operation. "
        "All patches are validated first; if any fail the entire operation is rolled back. "
        "Each entry must have 'path' and 'diff' keys."
    )
)
