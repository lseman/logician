...
try:
    llm  # type: ignore[used-before-def]
except NameError:
...
@llm.tool(
    description=(
        "Explicit pre-action planning checkpoint. Call this BEFORE any multi-step "
        "task to write out the problem, your approach, and the steps you plan to take. "
        "The plan is stored in the session scratch so it survives across turns. "
        "Use this whenever you would otherwise spend multiple turns stumbling — "
        "plan first, act second. Returns a confirmation with the stored plan."
    )
)
def think(
    problem: str,
...
@llm.tool(
    description=(
        "Retrieve previously recorded think-plans for this session. "
        "Useful when resuming after an interruption or when you want to "
        "review what was planned before continuing."
    )
)
