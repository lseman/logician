...
import type {
	...
	ExtensionContext,
	RegisteredCommand,
} from "../../packages/log-core/src/extensions.ts";
...
const GSD_COMMANDS: RegisteredCommand[] = [
	// ── Project Lifecycle ─────────────────────────────────────────────
	{
		...
		handler: async (
			args: string,
			ctx: { ui: ExtensionContext["ui"]; sessionId: string; cwd: string },
		) => {
		...
		handler: async (_args, ctx) => {
			const hasProject =
				...
				require("node:fs").existsSync(
					require("node:path").join(ctx.cwd, ".planning/PROJECT.md"),
				);
			const hasState =
				ctx.cwd &&
				require("node:fs").existsSync(
					require("node:path").join(ctx.cwd, ".planning/STATE.md"),
				);
			const phasesDir =
				ctx.cwd &&
				require("node:fs").existsSync(
					require("node:path").join(ctx.cwd, ".planning/phases"),
				);
			if (!hasProject)
				return "No project initialized. Run /gsd:new-project first.";
			if (!phasesDir)
				return "No phases found. Run /gsd:progress to start planning.";
			const phases = phase.listPhases(ctx.cwd);
			const nextPhase = phases.find(p => p.status === "unplanned");
			if (nextPhase)
				return `Next unplanned phase: ${nextPhase.phaseId} (${nextPhase.phaseName}). Run /gsd:discuss-phase ${nextPhase.phaseId}`;
			const inProgress = phases.find(p => p.status === "in-progress");
			if (inProgress)
				return `In-progress: ${inProgress.phaseId}. Run /gsd:progress or /gsd:execute-phase`;
	...
	// ── Phase Workflows ───────────────────────────────────────────────
	{
		...
		usage:
			"<phase> [--all] [--auto] [--chain] [--batch] [--analyze] [--text] [--power] [--assumptions]",
		acceptsArgs: true,
		handler: async (args, ctx) => {
	...
	},
	{
		...
		usage:
			"[phase] [--auto] [--research] [--skip-research] [--gaps] [--skip-verify] [--mvp] [--tdd]",
		acceptsArgs: true,
		handler: async (args, ctx) => {
			const phaseNum = args.split(" ")[0];
	...
	// ── Quick Tasks ───────────────────────────────────────────────────
	{
		...
		usage:
			"[list | status <slug> | resume <slug> | --full] [--validate] [--discuss] [--research] [task description]",
		acceptsArgs: true,
		handler: async (args, ctx) => {
			if (args.startsWith("list")) return `GSDBRIDGE:quick:list`;
			if (args.startsWith("status "))
				return `GSDBRIDGE:quick:status:${args.slice(7)}`;
			if (args.startsWith("resume "))
				return `GSDBRIDGE:quick:resume:${args.slice(7)}`;
	...
	// ── Project Management ────────────────────────────────────────────
	{
		name: "/gsd:progress",
		description:
			"Check progress, advance workflow, or dispatch freeform intent",
	...
	},
	{
		...
		description:
			"Generate a comprehensive project summary from milestone artifacts",
		usage: "",
		acceptsArgs: false,
		handler: async (_args, ctx) => {
	...
	// ── Review & Audit ────────────────────────────────────────────────
	{
		...
		description:
			"Review source files changed during a phase for bugs and quality",
		usage: "[phase]",
		acceptsArgs: true,
		handler: async (args, ctx) => {
	...
	},
	{
		name: "/gsd:audit-fix",
		description:
			"Autonomous audit-to-fix pipeline — find issues, classify, fix, test",
		usage: "[target]",
		acceptsArgs: true,
		handler: async (args, ctx) => {
	...
	// ── Memory & Context ──────────────────────────────────────────────
	{
		...
		description:
			"File a phase artifact into MemPalace; mirror decision facts into temporal KG",
		usage: "<artifact-path>",
		acceptsArgs: true,
		handler: async (args, ctx) => {
	...
	// ── Workstreams ───────────────────────────────────────────────────
	{
		...
		description:
			"Manage parallel workstreams — list, create, switch, status, complete",
	...
	},
	{
		name: "/gsd:workspace",
		description:
			"Manage GSD workspaces — create, list, or remove isolated environments",
	...
	// ── Additional Commands ───────────────────────────────────────────
	{
		name: "/gsd:explore",
		description:
			"Socratic ideation and idea routing — think through ideas before committing",
		usage: "[topic]",
		acceptsArgs: true,
		handler: async (args, ctx) => {
