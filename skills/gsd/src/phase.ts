...
export function getPhaseDir(cwd: string, phaseId: string): string {
	...
	if (!fs.existsSync(dir)) {
		throw new Error(
			`Phase ${phaseId} not found. Run /gsd:discuss-phase ${phaseId} first.`,
		);
	}
	return dir;
}
...
export function listPhases(cwd: string): PhaseInfo[] {
	...
	return result.sort(
		(a, b) => parseInt(a.paddedPhase, 10) - parseInt(b.paddedPhase, 10),
	);
}
...
export function getPhaseStatus(cwd: string, phaseId: string): string {
	...
	const phase = phases.find(
		p => p.phaseId === phaseId || p.paddedPhase === padPhase(phaseId),
	);
...
export function completePlanPhase(
	cwd: string,
	phaseId: string,
	summary: string,
): void {
