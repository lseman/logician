...
export function readState(cwd: string): { frontmatter: StateFrontmatter; body: string } {
...
function parseState(content: string): { frontmatter: StateFrontmatter; body: string } {
...
export function advancePlan(cwd: string, phaseId: string, planId: string): void {
  setStateField(cwd, 'currentPhase', phaseId);
...
export function addDecision(cwd: string, decision: string, context: string): void {
  const { frontmatter, body } = readState(cwd);
