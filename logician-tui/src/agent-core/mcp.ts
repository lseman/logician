        ...
        for (const [name, config] of Object.entries(configs)) {
            ...
            try {
                ...
                for (const def of defs) {
                    this.tools.push(createMcpTool(client, def));
                }
...
class StdioMcpClient implements McpClient {
    ...
    async initialize(): Promise<void> {
        ...
        this.proc = spawn(this.config.command, this.config.args || [], {
            cwd: this.config.cwd
                ? resolve(this.cwd, this.config.cwd)
                : this.cwd,
        ...
        this.proc.on("exit", (code, signal) => {
            ...
            const error = new Error(
                `MCP server exited (${exitStatus})${suffix}`,
            );
        ...
        await this.rpc("initialize", {
            ...
            capabilities: { tools: {} },
            clientInfo: { name: "logician-tui", version: "0.2.0" },
        });
...
class HttpMcpClient implements McpClient {
    ...
    private async send(
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `MCP HTTP ${response.status}: ${text.slice(0, 600)}`,
                );
            }
            if (id === undefined) return {};
...
function createMcpTool(client: McpClient, def: McpToolDefinition): Tool {
    ...
    return {
        ...
        description:
            def.description ||
            `MCP tool '${def.name}' from server '${client.name}'.`,
...
function findMcpConfig(cwd: string): string | null {
    ...
    while (true) {
        ...
        if (existsSync(agentConfig)) return agentConfig;
...
function encodeMcpMessage(message: JsonRpcRequest): Buffer {
