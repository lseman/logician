...
export function loadLogicianConfig(cwd = process.cwd()): ResolvedLogicianConfig {
...
export function findLogicianConfig(cwd = process.cwd()): string | null {
    ...
    if (envPath) {
        const resolved = resolve(envPath.replace(/^~(?=$|\/)/, process.env.HOME || ""));
