...
const NODES = {
  home: {
    title:"LOGICIAN", kicker:"guest@logician:~$ ls -la subsystems/",
    summary:"Click any box in the diagram — or press 1–5 — to enter it and see how it works.",
    color:"#22d3ee", type:"diagram", path:"~",
    children:["tui","harness","capabilities","coding-agent","external"]
  },

  tui:{
    title:"Terminal UI Layer", kicker:"drwxr-xr-x  tui/", path:"tui",
    color:"#8b5cf6", type:"grid",
    summary:"Everything the user sees and types. Rendering, input handling, theming, and modal overlays — the terminal-native front end (packages/tui).",
    children:["tui-rendering","tui-input","tui-themes","tui-overlays"]
  },
  "tui-rendering":{ title:"Rendering", kicker:"tui/rendering", color:"#8b5cf6", type:"leaf",
    summary:"Draws the live transcript on screen.",
    bullets:["Differential rendering — only the changed region of the terminal is redrawn, so nothing flickers.",
  ...
  "tui-input":{ title:"Input", kicker:"tui/input", color:"#8b5cf6", type:"leaf",
    summary:"Captures what the user types and turns it into messages for the harness.",
    bullets:["Multi-line editing with autocomplete for file paths and slash commands.",
  ...
  "tui-themes":{ title:"Themes", kicker:"tui/themes", color:"#8b5cf6", type:"leaf",
    summary:"Terminal color schemes.",
    bullets:["Defines the palette this very site borrows for its subsystem colors.",
      "Swappable independently of layout — the terminal chrome stays constant."]},
  "tui-overlays":{ title:"Overlays", kicker:"tui/overlays", color:"#8b5cf6", type:"leaf",
    summary:"Modal surfaces drawn on top of the transcript.",
    bullets:["ask_user prompts pause the loop and wait for an answer here.",
      "Permission confirmations for acceptEdits / ask permission modes.",
      "File pickers and other transient UI."]},

  harness:{
    title:"Harness — Agent Loop", kicker:"drwxr-xr-x  agent-core/", path:"agent-core",
    color:"#22d3ee", type:"pipeline",
    summary:"The loop that orchestrates every model call and tool execution. 10 stages, two nested loops.",
    children:["s1","s2","s3","s4","s5","s6","s7","s8","s9","s10"]
  },
  s1:{ n:1, stype:"pipeline", title:"Message Queue", kicker:"agent-core/message-queue", color:"#22d3ee", type:"leaf",
    summary:"Where a turn begins.",
    bullets:["Buffers the user's new message together with any steering or follow-up messages sent mid-run.",
  ...
  s2:{ n:2, stype:"pipeline", title:"Context Assembly", kicker:"agent-core/context", color:"#22d3ee", type:"leaf",
    summary:"Builds what the model will actually see this turn.",
    bullets:["Combines the system prompt, loaded context files, and the session transcript so far.",
      "Pulls in cross-session context from agent memory."]},
  s3:{ n:3, stype:"gate", title:"Compaction Check", kicker:"agent-core/compaction", color:"#06b6d4", type:"leaf",
    summary:"A gate, not a stage that always fires.",
    bullets:["If compaction is enabled and the transcript is near the configured reserveTokens limit, older turns are summarized first.",
      "Otherwise this is a no-op and the turn proceeds untouched."]},
  s4:{ n:4, stype:"pipeline", title:"Model Call", kicker:"agent-core/model-call", color:"#22d3ee", type:"leaf",
    summary:"Streams the request to the configured LLM backend.",
    bullets:["Talks to any OpenAI-compatible API.",
  ...
  s5:{ n:5, stype:"gate", title:"Parse Response", kicker:"agent-core/parse", color:"#06b6d4", type:"leaf",
    summary:"The loop's real branch point.",
    bullets:["Splits the model's response into plain text and tool calls.",
  ...
  s6:{ n:6, stype:"pipeline", title:"Pre-Tool Hooks", kicker:"agent-core/hooks/pre", color:"#22d3ee", type:"leaf",
    summary:"Before a tool actually runs, registered extensions get a chance to act.",
    bullets:["Can intercept, modify, or block the call entirely.",
      "Claude-style lifecycle hooks — the same extension point skills and plugins build on."]},
  s7:{ n:7, stype:"gate", title:"Permission Gate", kicker:"agent-core/permissions", color:"#06b6d4", type:"leaf",
    summary:"Checks the active permission mode before anything executes.",
    bullets:["acceptAll — execute everything without asking.",
  ...
  s8:{ n:8, stype:"tool", title:"Tool Execution", kicker:"agent-core/tools", color:"#34d399", type:"leaf",
    summary:"Dispatches to the tool registry.",
    bullets:["File ops, search, system commands, agent primitives, and web & docs tools.",
  ...
  s9:{ n:9, stype:"pipeline", title:"Post-Tool Hooks", kicker:"agent-core/hooks/post", color:"#22d3ee", type:"leaf",
    summary:"Formats the result before it re-enters the transcript.",
    bullets:["For JS/TS/JSON edits, runs a fast syntax check before moving on (post-edit diagnostics).",
      "Last chance for an extension to reshape a tool result."]},
  s10:{ n:10, stype:"mem", title:"Append to Session", kicker:"agent-core/session", color:"#22d3ee", type:"leaf",
    summary:"Writes the tool result into the transcript.",
    bullets:["Pending writes are flushed at defined save points, not on every partial update.",
      "This is what agent memory persists across sessions."]},

  capabilities:{
    title:"Agent Capabilities", kicker:"drwxr-xr-x  agent-capabilities/", path:"agent-capabilities",
    color:"#06b6d4", type:"grid",
    summary:"Higher-level behaviors the harness can call on: task tracking, user prompts, child agents, and structured reasoning.",
    children:["cap-todo","cap-ask-user","cap-subagents","cap-reasoners","cap-eoh"]
  },
  "cap-todo":{ title:"todo", kicker:"agent-capabilities/todo", color:"#06b6d4", type:"leaf",
    summary:"Task tracking with status transitions.",
    bullets:["Lets the agent break a request into steps and report progress on each one.",
      "Visible in the TUI as the run progresses."]},
  "cap-ask-user":{ title:"ask-user", kicker:"agent-capabilities/ask-user", color:"#06b6d4", type:"leaf",
    summary:"User input prompts.",
    bullets:["Pauses the loop to ask the user a direct question through a TUI overlay.",
      "Resumes the loop with the answer once given."]},
  "cap-subagents":{ title:"subagents", kicker:"agent-capabilities/subagents", color:"#06b6d4", type:"leaf",
    summary:"Child agents, isolated worktrees.",
    bullets:["spawn_agent starts a child agent with a fresh context window.",
  ...
  "cap-reasoners":{ title:"reasoners", kicker:"agent-capabilities/reasoners", color:"#06b6d4", type:"leaf",
    summary:"Structured reasoning strategies.",
    bullets:["SSR, Tree of Thoughts, and Reflexion — alternatives to a single-shot model call.",
      "Plug into the Model Call stage of the harness loop."]},
  "cap-eoh":{ title:"eoh", kicker:"agent-capabilities/eoh", color:"#06b6d4", type:"leaf",
    summary:"An additional capability module exported alongside the others.",
    bullets:["Exported from packages/agent-capabilities."]},

  "coding-agent":{
    title:"Coding Agent", kicker:"drwxr-xr-x  coding-agent/", path:"coding-agent",
    color:"#34d399", type:"grid",
    summary:"The full coding-agent runtime built on top of the harness: built-in tools, skills, MCP support, and sessions.",
    children:["ca-tools","ca-skills","ca-mcp","ca-context","ca-prompts","ca-sessions"]
  },
  "ca-tools":{ title:"Tools", kicker:"coding-agent/tools", color:"#34d399", type:"leaf",
    summary:"Built-in tools the model can call directly.",
    bullets:["File ops: read, write, edit.",
      "Search: grep-style content search across the repo.",
      "System: bash and other shell-level commands.",
      "Web & docs: web_search / web_fetch."]},
  "ca-skills":{ title:"Skills", kicker:"coding-agent/skills", color:"#34d399", type:"leaf",
    summary:"SKILL.md-driven capabilities.",
    bullets:["Best-practice playbooks the agent reads before touching a given file type or task.",
      "Loaded on demand rather than kept in the system prompt at all times."]},
  "ca-mcp":{ title:"MCP client", kicker:"coding-agent/mcp", color:"#34d399", type:"leaf",
    summary:"Connects to external MCP servers.",
    bullets:["Supports both stdio and streamable HTTP transports.",
      "Exposes each server's tools to the harness's Tool Execution stage as if they were local."]},
  "ca-context":{ title:"Context files", kicker:"coding-agent/context-files", color:"#34d399", type:"leaf",
    summary:"Loads repository and documentation context.",
    bullets:["Feeds into Context Assembly in the harness loop alongside the system prompt."]},
  "ca-prompts":{ title:"Prompts + Trust", kicker:"coding-agent/prompts", color:"#34d399", type:"leaf",
    summary:"System prompts and permission modes.",
    bullets:["Defines what a tool call is allowed to do before the Permission Gate checks it.",
      "acceptAll / acceptEdits / ask / plan are configured here."]},
  "ca-sessions":{ title:"Sessions", kicker:"coding-agent/sessions", color:"#34d399", type:"leaf",
    summary:"Session persistence, bookmarks, and rewind.",
    bullets:["The same store Append to Session writes into on every turn.",
      "Bookmarks mark a point to return to; rewind restores an earlier checkpoint."]},

  external:{
    title:"External Systems", kicker:"drwxr-xr-x  external/", path:"external",
    color:"#64748b", type:"grid",
    summary:"Systems Logician talks to outside its own process — the model, other tool servers, and the local machine.",
    children:["ext-llm","ext-mcp","ext-searxng","ext-fs"]
  },
  "ext-llm":{ title:"LLM Backend", kicker:"external/llm-backend", color:"#64748b", type:"leaf",
    summary:"OpenAI-compatible API.",
    bullets:["The configured model that actually generates responses and tool calls.",
      "Called from the harness's Model Call stage."]},
  "ext-mcp":{ title:"MCP Servers", kicker:"external/mcp-servers", color:"#64748b", type:"leaf",
    summary:"stdio & streamable HTTP.",
    bullets:["Any MCP server reachable this way exposes extra tools to the agent.",
      "Reached through the Coding Agent's MCP client."]},
  "ext-searxng":{ title:"SearXNG", kicker:"external/searxng", color:"#64748b", type:"leaf",
    summary:"Self-hosted web search.",
    bullets:["Backs the web_search / web_fetch tools.",
      "No dependency on a third-party search API."]},
  "ext-fs":{ title:"Filesystem + Git", kicker:"external/fs-git", color:"#64748b", type:"leaf",
    summary:"Direct access to the local machine.",
    bullets:["bash, grep (rg), find (fd), and git.",
...
function current(){ return node(path[path.length-1]); }
function parentOf(id){
  for(const k in NODES){ if(NODES[k].children && NODES[k].children.includes(id)) return k; }
  return null;
}
...
function finishBoot(){
  ...
  setTimeout(()=>{ bootEl.style.display="none"; renderView(); }, 360);
}
...
(function typeBoot(){
  ...
  const t = setInterval(()=>{
    ...
    if(i>=bootText.length){ clearInterval(t); setTimeout(finishBoot, 700); }
  }, 45);
})();
bootEl.addEventListener("click", finishBoot);
window.addEventListener("keydown", (e)=>{ if(!bootDone){ finishBoot(); } }, {once:false});
...
function findEl(id){
  return viewEl.querySelector(`[data-nav="${id}"]`) || viewEl.querySelector(`[data-id="${id}"]`);
}

/* ---------- generic sub-architecture: hub + spoke ---------- */
function renderSubArch(n){
  const items = n.children.map(id=>({id, ...node(id)}));
  const nodesHTML = items.map(it => `
...
function drawConnectors(color){
  ...
  boxes.forEach(b=>{
    ...
    centers.push({cx:r.left+r.width/2-wrapRect.left, topY:r.top-wrapRect.top});
  });
  let s = "";
  if(centers.length>1){
...
function stageColor(stype){
  return stype==="tool" ? "#34d399" : stype==="mem" ? "#22d3ee" : stype==="gate" ? "#06b6d4" : "#22d3ee";
}
...
function renderHomeDiagram(n){
  const svgContent = HOME_SVG || `<div style="padding:40px;color:var(--dim);font-family:var(--mono)">loading diagram…</div>`;
  return `
    <div class="eyebrow" style="color:${n.color}">${n.kicker}</div>
    <h1 class="title">${n.title}</h1>
    <p class="summary">${n.summary}</p>
...
function renderView(){
  ...
  if(n.type==="diagram"){ viewEl.innerHTML = renderHomeDiagram(n); }
  else if(n.type==="grid"){ viewEl.innerHTML = renderSubArch(n); }
  else { viewEl.innerHTML = renderPipeline(n); }
  viewEl.scrollTop = 0;
  attachHandlers();
  if(n.type==="grid"){
...
function attachHandlers(){
  viewEl.querySelectorAll(".prow, .subnode").forEach(el=>{
    ...
    el.addEventListener("keydown", (e)=>{
      if(e.key==="Enter" || e.key===" "){ e.preventDefault(); activate(id, el); }
    });
  });
  ...
  ttEl.querySelectorAll(".chip").forEach(chip=>{
    ...
    chip.addEventListener("keydown", (e)=>{
      if(e.key==="Enter" || e.key===" "){ e.preventDefault(); jumpToRelated(chip); }
    });
  });
  ...
  if(svg){
    ...
    svg.querySelectorAll("[data-nav]").forEach(el=>{
      ...
      el.addEventListener("keydown", (e)=>{
        if(e.key==="Enter" || e.key===" "){ e.preventDefault(); activate(el.getAttribute("data-nav"), el); }
      });
    });
  }
}
...
function tooltipContentHTML(n, id){
  ...
  const related = (n.related||[]).map(([label,color])=>
    `<span class="chip" style="border-color:${color}66; color:${color}">${label}</span>`).join("");
  let dots = "";
...
function showTooltip(id, el){
  ...
  let top = r.bottom + 14;
  if(top + tr.height > window.innerHeight - 12){ top = r.top - tr.height - 14; }
  top = Math.max(60, top);
...
function activate(id, el){
  const n = node(id);
  if(n.type === "leaf"){ showTooltip(id, el); }
  else { hideTooltip(); enter(id, el); }
}
...
function enter(id, originEl){
  ...
  const doPush = ()=>{ path.push(id); renderView(); };
  if(originEl){ zoomIn(originEl, color, doPush); }
  else { doPush(); }
}
...
function back(){
  ...
  zoomOut(leavingColor, ()=>{ path.pop(); renderView(); });
}

function jumpTo(index){
  if(index >= path.length-1) return;
  const leavingColor = current().color;
  zoomOut(leavingColor, ()=>{ path = path.slice(0,index+1); renderView(); });
}
...
function jumpDigit(n){
  ...
  const el = findEl(id) || document.querySelector(`#home-svg [data-nav="${id}"]`);
  activate(id, el);
}
...
function openSearch(){
  ...
  document.getElementById("search-results").innerHTML = "<div style='padding:20px;text-align:center;color:var(--dim);font-family:var(--mono);font-size:12px;'>start typing to search...</div>";
}
...
function doSearch(query){
  ...
  if(!query || query.length < 2){
    results.innerHTML = "<div style='padding:20px;text-align:center;color:var(--dim);font-family:var(--mono);font-size:12px;'>type at least 2 chars...</div>";
    return;
  }
  ...
  for(const id in NODES){
    const n = NODES[id];
    const searchable = (n.title + " " + n.summary + " " + (n.kicker||"") + " " + id).toLowerCase();
    if(searchable.includes(q)){
      matches.push({ id, score: n.title.toLowerCase().startsWith(q) ? 0 : 1, ...n });
    }
  }
  matches.sort((a,b) => a.score - b.score);
  if(matches.length === 0){
    results.innerHTML = "<div style='padding:20px;text-align:center;color:var(--dim);font-family:var(--mono);font-size:12px;'>no matches found</div>";
    return;
  }
...
window.addEventListener("keydown", (e)=>{
  ...
  if(helpEl.classList.contains("show") && e.key==="Escape"){ helpEl.classList.remove("show"); return; }
  if(e.key==="Escape" || e.key==="Backspace"){
    e.preventDefault();
    if(activeTooltip){ hideTooltip(); } else { back(); }
    return;
  }
  if(e.key==="ArrowRight"){ if(!tooltipSibling(1)) return; return; }
  if(e.key==="ArrowLeft"){ if(!tooltipSibling(-1)) return; return; }
  if(e.key==="Home"){ hideTooltip(); path=["home"]; renderView(); return; }
  if(/^[1-9]$/.test(e.key)){ jumpDigit(parseInt(e.key,10)); return; }
