...
/** Details stored in branch summary for file tracking. */
...
const BRANCH_SUMMARY_PREAMBLE = `[Branch Summary]`;
...
export interface GenerateBranchSummaryOptions {
	/** Generate summary using LLM (true) or parse from existing text (false). */
	useLLM?: boolean;
	/** Max tokens for token-budgeted collection. */
	tokenBudget?: number;
	/** Custom instructions appended to the prompt. */
	customInstructions?: string;
	/** Replace default prompt entirely with custom instructions. */
	replaceInstructions?: boolean;
}

/**
 * Generate a structured branch summary from messages.
 *
 * If messages are provided, extracts file ops and calls generateBranchSummaryText.
 * If customInstructions is provided, appends or replaces the default prompt.
 *
 * @param messages - Messages in the branch to summarize
 * @param options - Generation options
 * @returns Structured branch summary with file tracking
 */
export function generateBranchSummaryText(
	messages: Message[],
	options?: GenerateBranchSummaryOptions,
): BranchSummary {
	const fileOps = extractFileOpsFromMessages(messages);
	...
	// Prepend preamble
	const preamble = `${BRANCH_SUMMARY_PREAMBLE}\n`;

	// Append file operations
	const fileOpsText = formatFileOperations(readFiles, modifiedFiles);

	const full = `${preamble}${promptText}${fileOpsText}`;

	// Return a basic structured summary — the LLM call fills in the real content
	const parsed = parseBranchSummary(full);
...
/**
 * Generate branch summary text for LLM consumption.
 * Returns the serialized messages wrapped in the branch summary prompt.
 */
export function generateBranchSummaryTextForLLM(
	messages: Message[],
	options?: { customInstructions?: string; replaceInstructions?: boolean },
): { messages: Array<{ role: string; content: string }>; fileOps: FileOperations } {
	const fileOps = extractFileOpsFromMessages(messages);

	const parsed = parseBranchSummary(generateBranchSummaryText(messages, options).full);

	// For LLM calls, we need the prompt messages, not the full summary
