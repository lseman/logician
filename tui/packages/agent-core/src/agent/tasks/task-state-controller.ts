...
export function formatTaskStateContext(state: ExplicitTaskState): string {
	...
	return [
		...
		"recent_evidence:",
		...(recent.length
			? recent.map((item) => `- [${item.kind}] ${item.tool ? `${item.tool}: ` : ""}${item.summary}`)
			: ["- none"]),
...
const WRITE_TOOLS = new Set(["edit_file", "write_file", "write_file_append", "apply_patch"]);
const READ_TOOLS = new Set(["read_file", "grep", "glob", "list_files", "web_search"]);
const VERIFY_PATTERN = /(?:^|\b)(?:test|check|lint|typecheck|build|compile|pytest|vitest|jest|cargo test)(?:\b|$)/i;
const FAILURE_PATTERN = /(?:\bfailed\b|\bfailure\b|\b[1-9]\d* fails?\b|\berror(?:\s*:|$)|exception|traceback|not ok|exit(?:ed)? (?:code )?[1-9])/i;
...
export class TaskStateController {
	...
	recordToolBatch(
		...
		for (let index = 0; index < calls.length; index++) {
			...
			if (failed) {
				...
				this.pushEvidence("failure", result || `${call.name} failed`, call.name, iteration);
			} else if (WRITE_TOOLS.has(call.name)) {
				const file = stringArg(args, "path", "file", "filePath");
				if (file && !this.state.changedFiles.includes(file)) this.state.changedFiles.push(file);
				this.pushEvidence("change", result || `${call.name} completed`, call.name, iteration);
			} else {
				this.pushEvidence("observation", result || `${call.name} completed`, call.name, iteration);
			}

			if (call.name === "bash" && VERIFY_PATTERN.test(command)) {
				this.state.verification.push({ command, passed: !failed, summary: result });
			}
		}
	...
	markBlocked(reason: string): void {
		...
		if (summary && !this.state.blockers.includes(summary)) this.state.blockers.push(summary);
	...
	selectAdaptiveMode(): AdaptiveModeDecision {
		...
		if (this.state.phase === "blocked" || this.state.toolFailures >= 2) {
			return { mode: "thinking-coding", reason: "recovery after repeated tool failures or a blocker" };
		}
		if (this.state.phase === "verify") {
			return { mode: "deterministic", reason: "verification favors reproducible output" };
		}
		if (this.state.phase === "implement") {
			return { mode: "instruct-coding", reason: "implementation phase favors precise code generation" };
		}
		if (/\b(?:brainstorm|creative|ideas?|name|design alternatives?)\b/.test(objective)) {
			return { mode: "creative", reason: "objective requests ideation or alternatives" };
		}
		if (/\b(?:debug|diagnos|review|analy[sz]|compare|investigat|why)\b/.test(objective)) {
			return { mode: "analytical", reason: "objective is primarily diagnostic or analytical" };
		}
		if (/\b(?:implement|fix|code|refactor|build|add|change|test)\b/.test(objective)) {
			return { mode: "thinking-coding", reason: "coding objective still requires orientation" };
		}
		return { mode: "instruct-general", reason: "general objective with no elevated reasoning signal" };
	}
	...
	private pushEvidence(
		...
		this.state.evidence.push({ kind, summary: compact(summary), tool, iteration });
