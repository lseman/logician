...
const CONFIG_FILE_NAME = "autoresearch.json";
...
function getLogicianAgentDir(): string {
	return join(os.homedir(), ".logician");
}

export function autoresearchShortcutsConfigPath(
	agentDir: string = getLogicianAgentDir(),
): string {
...
function readShortcutConfig(
	configPath: string,
): AutoresearchShortcutConfig | null {
	let parsed: unknown;
	try {
...
function isValidShortcutConfigValue(
	value: unknown,
...
function warnUsingDefaults(
	reason: "Could not read" | "Invalid",
	configPath: string,
): void {
	console.warn(
		`${reason} autoresearch config at ${configPath}; using default shortcuts.`,
	);
}
