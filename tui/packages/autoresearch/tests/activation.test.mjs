...
test("same-cwd persisted logs still auto-activate autoresearch", () => {
	assert.equal(shouldAutoActivateAutoresearch("/repo", "/repo", true), true);
});

test("missing persisted logs never auto-activate autoresearch", () => {
	assert.equal(shouldAutoActivateAutoresearch("/repo", "/repo", false), false);
});
...
test("starting autoresearch binds redirected workingDir activation to the pi session", async () => {
	...
	try {
		...
		await harness.commands
			.get("autoresearch")
			.handler("optimize runtime", harness.ctx);
		...
		assert.equal(
			harness.appendedEntries[0].data.workDir,
