...
describe("createMemoryHooks observation capture", () => {
	test("retrieves memory against live task state rather than only the initial prompt", async () => {
		...
		const injected = String(
			last && "content" in last ? last.content || "" : "",
		);
	...
	test("replaces stale retrieved context instead of accumulating system blocks", async () => {
		const store = testStore();
		store.create("Authentication uses bounded exponential retries", {
			strength: 8,
		});
		const hooks = createMemoryHooks(store, { contextBudget: 1000 });
		await hooks.beforeAgentStart?.({
			prompt: "Fix authentication retries",
			systemPrompt: "",
			messages: [],
		});

		const first = await hooks.transformContext?.({
			messages: [],
			iteration: 1,
		});
	...
	test("captures prompts and tool outcomes without duplicating pre-tool intent", async () => {
		...
		const promptObservation = observations.find(
			observation => observation.type === "conversation",
		);
		assert.match(
			promptObservation?.title || "",
			/Fix the authentication timeout/,
		);
		assert.equal(
			promptObservation?.narrative,
			"Fix the authentication timeout",
		);
		assert.deepEqual(
	...
	test("deduplicates repeated prompts and equivalent tool calls", async () => {
		...
		await hooks.afterToolCall?.({
			...tool,
			toolCall: { ...tool.toolCall, id: "call-b" },
		});
	...
	test("consolidates high-signal observations at a turn boundary and notifies", async () => {
		...
		await hooks.afterProviderResponse?.({
			model: "test",
			content:
				"Authentication now uses a bounded timeout in /workspace/auth.ts.",
			toolCallCount: 0,
			stopReason: "stop",
			iteration: 1,
		});
		await hooks.shouldStopAfterTurn?.({
			messages: [],
			iteration: 1,
			hadToolCalls: false,
		});
	...
	test("synthesizes a grounded turn episode from intent, mutations, outcome, and verification", async () => {
		...
		await hooks.afterProviderResponse?.({
			model: "test",
			content:
				"Session history is now stored in each workspace's .logician directory.",
			toolCallCount: 0,
			stopReason: "stop",
			iteration: 2,
		});
	...
	test("accepts specific model claims only when evidence and verification are grounded", async () => {
		...
		const hooks = createMemoryHooks(store, {
			...
			semanticExtractor: async () =>
				JSON.stringify({
					...
					summary:
						"The authentication timeout path was corrected and its focused test completed successfully.",
					claims: [
						{
		...
		await hooks.afterToolCall?.({
			...
			args: { path: "/workspace/auth.ts" },
			result: "Updated timeout policy",
			isError: false,
			iteration: 0,
		});
		await hooks.afterToolCall?.({
			toolCall: { id: "test-grounded", name: "exec_command", arguments: "{}" },
			args: { command: "bun test auth.test.ts" },
			result: "1 pass, 0 fail",
			isError: false,
			iteration: 1,
		});
		...
		assert.equal(
			episode?.title,
			"Authentication timeout now uses a bounded retry policy",
		);
		assert.match(
			episode?.facts[0] || "",
	...
	test("rejects hallucinated model evidence and falls back to deterministic synthesis", async () => {
		...
		const hooks = createMemoryHooks(store, {
			...
			semanticExtractor: async () => ({
				...
				summary:
					"A deployment workflow was supposedly added to an unrelated file and verified.",
				claims: [
					{
						text: "Production deploys now use canaries.",
						confidence: 1,
						status: "verified",
				...
				filesRead: [],
				filesModified: ["/invented/deploy.ts"],
				concepts: [],
			}),
		});
		await hooks.beforeAgentStart?.({
			prompt: "Update auth timeout",
			systemPrompt: "",
			messages: [],
		});
		await hooks.afterToolCall?.({
			...
			args: { path: "/workspace/auth.ts" },
			result: "Updated timeout",
			isError: false,
			iteration: 0,
		});
		await hooks.afterProviderResponse?.({
			model: "test",
			content: "Authentication timeout is now bounded.",
			toolCallCount: 0,
			stopReason: "stop",
			iteration: 1,
		});
	...
	test("honors a model skip for turns without durable knowledge", async () => {
		...
		assert.equal(
			store
				.listObservations("session-1", 10)
				.some(item => item.id.startsWith("episode:")),
			false,
		);
		store.close();
	});
	...
	test("runs semantic extraction in the background and serializes turns", async () => {
		...
		const firstResult = hooks.shouldStopAfterTurn?.({
			messages: [],
			iteration: 0,
			hadToolCalls: false,
		});
		...
		const secondResult = hooks.shouldStopAfterTurn?.({
			messages: [],
			iteration: 1,
			hadToolCalls: false,
		});
