...
const VERIFICATION_TOOL = /(?:test|check|lint|typecheck|build|compile|verify|pytest|vitest|jest|cargo|go test)/i;
...
function collectPaths(value: unknown, output: Set<string>, depth = 0): void {
  ...
  if (typeof value === "string") {
    for (const match of value.matchAll(/(?:^|[\s"'`])((?:\.?\.?\/|\/)?[\w.@+-]+(?:\/[\w.@+-]+)+\.[\w+-]+)/g)) {
  ...
  if (typeof value === "object") {
    for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
      if (/(?:path|file|filename|target)/i.test(key) && typeof item === "string" && item.trim()) {
...
function inferEpisodeType(evidence: TurnEvidence, changed: boolean, failed: boolean): ObservationType {
  const text = `${evidence.userIntent} ${evidence.assistantOutcome}`.toLowerCase();
  if (failed && !changed) return "error";
  if (/\b(?:decid|chose|choice|trade-?off|rationale|architecture)\b/.test(text)) return "decision";
  if (/\b(?:bug|fix|broken|regression|crash|incorrect|failure)\b/.test(text) && changed) return "bugfix";
  if (changed) return "implementation";
  if (/\b(?:discover|found|identified|confirmed|traced|root cause|learned)\b/.test(text)) return "discovery";
  return "conversation";
}
...
export function synthesizeTurnEpisode(evidence: TurnEvidence): {
  ...
  const changedTools = evidence.tools.filter((tool) => WRITE_TOOL.test(tool.name));
  ...
  const type = inferEpisodeType(evidence, changedTools.length > 0, failedTools.length > 0);
  const outcomeLine = cleanLine(outcome);
  const intentLine = cleanLine(intent) || intent.slice(0, 160);
  const title = outcomeLine && !/^(?:done|completed|implemented|fixed|updated)\.?$/i.test(outcomeLine)
    ? outcomeLine
  ...
  if (paths.size) facts.push(`${changedTools.length ? "Relevant files" : "Files inspected"}: ${[...paths].slice(0, 12).join(", ")}`);
  if (verification.length) {
    const passed = verification.filter((tool) => !tool.isError && !/\b[1-9]\d*\s+(?:fail(?:ed|ure)?|errors?)\b|\b(?:tests?|checks?)\s+failed\b/i.test(tool.result));
    facts.push(`Verification: ${passed.length}/${verification.length} checks completed without reported failure (${verification.map((tool) => tool.name).join(", ")})`);
  }
  if (failedTools.length) facts.push(`Failed evidence: ${failedTools.map((tool) => `${tool.name}: ${cleanLine(tool.result) || "failed"}`).join("; ")}`);
  ...
  const importance = type === "decision" || type === "bugfix" ? 9 : changedTools.length ? 8 : failedTools.length ? 8 : 6;
