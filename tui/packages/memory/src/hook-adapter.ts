...
import type { MemoryStore, MemoryQuery, MemoryType, RecallOptions, WorkingMemoryTier, AutoForgetConfig } from "./types.js";
...
export function registerMemoryHooks(
  ...
  const maxObs = options?.maxRecentObservations ?? 10;
  ...
  bus.on("beforeCompact", (ctx: any, signal?: AbortSignal) => {
    ...
    try {
      ...
      if (newMemories.length > 0) {
        console.log(`[memory] Consolidated ${newMemories.length} memories for session ${ctx.sessionId.slice(0, 12)}`);
      }
    } catch (e) {
      console.error(`[memory] Consolidation failed: ${e instanceof Error ? e.message : String(e)}`);
    }
  ...
  if (captureTools) {
    bus.on("postToolUse", (data: any) => {
      ...
      store.observe({
        ...
        toolOutput,
        workspace,
        raw: { tool_name: toolName, tool_input: toolInput, tool_output: toolOutput },
      });
    });
...
export function listMemories(
  ...
  const memories = store.list({ ...query, type: (query?.type as MemoryType | undefined) ?? undefined, limit });
