...
import type { Turn, AssistantMessage, UserMessage, AssistantChunk, ToolExecution } from "@logician/coding-agent/sessions";
...
function genRandomText(length: number, seed: number): string {
	const words = ["the","function","variable","component","render","update","handle","process","compute"];
	let text = "";
	let n = seed;
...
function fullCommitFrameDiff(prevLines: string[], newLines: string[]): void {
	const renderWidth = Math.max(1, TOTAL_COLS);
	for (let row = 0; row < termHeight; row++) {
		const prevLine = prevLines[row];
		const newLine = row < newLines.length ? newLines[row] : " ".repeat(termHeight);
...
function measureBlock(name: string, fn: () => void): number {
...
function main() {
	for (const { label, numTurns, chunkSize } of [
		...
		for (let i = 0; i < NUM_KEYSTROKES; i++) {
		 ...
		 // handleInput
		 buckets.handleInput.push(measureBlock("hi", () => inputBar.handleInput(key)));

		 // renderLayoutFrame (includes flex layout + all component renders)
		 buckets.layout.push(measureBlock("layout", () => {
			 frame = renderLayoutFrame(root, TOTAL_COLS, termHeight, () => {});
		 }));

		 // full diff
		 buckets.diff.push(measureBlock("diff", () => fullCommitFrameDiff(prevLines, frame.lines)));
	 ...
	 console.log(`  ${"handleInput".padEnd(16)}: p50=${stats(buckets.handleInput).p50}ms p95=${stats(buckets.handleInput).p95}ms avg=${stats(buckets.handleInput).avg}ms`);
	 console.log(`  ${"renderLayoutFrame".padEnd(16)}: p50=${stats(buckets.layout).p50}ms p95=${stats(buckets.layout).p95}ms avg=${stats(buckets.layout).avg}ms`);
	 console.log(`  ${"fullDiff".padEnd(16)}: p50=${stats(buckets.diff).p50}ms p95=${stats(buckets.diff).p95}ms avg=${stats(buckets.diff).avg}ms`);
	 console.log(`  ${"TOTAL".padEnd(16)}: p50=${stats(buckets.fullKeystroke).p50}ms p95=${stats(buckets.fullKeystroke).p95}ms avg=${stats(buckets.fullKeystroke).avg}ms`);
	 ...
	 console.log(`  Distribution: handleInput=${(inputSum/totalSum*100).toFixed(1)}% renderLayout=${(layoutSum/totalSum*100).toFixed(1)}% diff=${(diffSum/totalSum*100).toFixed(1)}%`);
	 ...
	 const tdTimes = Array.from({length: 200}, () => measureBlock("td_render", () => td.render(TOTAL_COLS)));
	 console.log(`  TranscriptDisplay.render(): p50=${stats(tdTimes).p50}ms avg=${stats(tdTimes).avg}ms`);
	}
}
