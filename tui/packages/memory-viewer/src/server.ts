...
export function startViewerServer(opts: ViewerOptions): {
	...
	function getHealth(): HealthStats {
		...
		return { rss: mem.rss, heapUsed: mem.heapUsed, heapTotal: mem.heapTotal, eventLoopLag: 0, uptime: Date.now() };
	}
	...
	async function handleRequest(req: Request): Promise<Response> {
		...
		if (path.startsWith("/api/")) {
			...
			if (!checkAuth(auth)) return new Response("Unauthorized", { status: 401 });
			...
			if (segments[0] === "sessions" && segments.length === 1) {
				return new Response(JSON.stringify(store.listSessions()), { headers: { "Content-Type": "application/json" } });
			}
			...
			if (segments[0] === "memories" && segments.length === 1) {
				...
				const memories = store.list({ search, type: type as any, minStrength, limit });
			...
			if (segments[0] === "auto-forget") {
				...
				const result = store.autoForget(body.ttlMs as number, body.minImportance as number, body.maxDeletes as number);
				return new Response(JSON.stringify(result), { headers: { "Content-Type": "application/json" } });
			}
			...
			if (segments[0] === "working-memory") {
				...
				for (const m of memories) {
					tiered[m.id] = { tier: store.getWorkingMemoryTier(m.id), strength: m.strength, type: m.type, content: m.content.slice(0, 100) };
				}
				return new Response(JSON.stringify(tiered), { headers: { "Content-Type": "application/json" } });
			}
			...
			if (segments[0] === "activity") {
				...
				for (const session of sessions) {
					...
					for (const o of obs) {
						activity.push({ type: "observation", sessionId: session.id, sessionProject: session.project, observation: o, timestamp: o.timestamp });
					}
				}
				activity.sort((a: any, b: any) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
				return new Response(JSON.stringify(activity.slice(0, limit)), { headers: { "Content-Type": "application/json" } });
			}

			if (segments[0] === "audit") {
				const limit = parseInt(url.searchParams.get("limit") || "100");
				const memoryList = store.list({ limit: limit * 2 });
				const audit = memoryList.map((m: any) => ({ id: m.id, operation: "create", resource: "memory", type: m.type, timestamp: m.createdAt, strength: m.strength }));
		...
		if (path === "/" || path === "/dashboard") {
			return new Response(documentHTML, { headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-cache" } });
		}
	...
	const wrapped = { port: server.port ?? port, stop: (force?: boolean) => server.stop(force) };
	return { server: wrapped, stop: (force?: boolean) => { server.stop(force); boundPort = null; } };
}
