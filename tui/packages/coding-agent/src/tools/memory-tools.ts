...
export function createMemorySearchTool(getStore: () => MemoryStore | null): Tool {
  return {
    name: "memory_search",
    label: "Search Memory",
    readOnly: true,
    executionMode: "parallel",
    description: "Search this workspace's durable memories and observations, returning compact summaries with stable IDs.",
    promptSnippet: "Search memory indexes for relevant IDs before expanding full records",
    promptGuidelines: [
    ...
    parameters: {
      type: "object",
      properties: {
        query: { type: "string", description: "Words or concepts to find in workspace memory" },
        limit: { type: "number", minimum: 1, maximum: 20, description: "Maximum results of each kind (default 8)" },
      },
      required: ["query"],
    },
    execute: async (args) => {
      ...
      const results = [
        ...
        ...observations.map(({ observation }) => {
          const description = brief([observation.narrative, ...observation.facts].filter(Boolean).join(" "));
          return `- [${observation.id}] Observation · ${observation.type} · ${observation.title}${description ? ` — ${description}` : ""}`;
        }),
      ];
      if (!results.length) return `No memory entries matched "${query}" in this workspace.`;
      return [
        `# Memory search: ${query}`,
        ...results,
