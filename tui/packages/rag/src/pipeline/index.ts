...
import type { ExtractedDocument, RAGChunk, IVectorStore } from "../types.ts";
...
const DEFAULT_SCRIPT_PATH = path.join(REPO_ROOT, "rag-python", "src", "rag_extract", "cli.py");
...
export class RAGPipeline {
  ...
  constructor(config: RAGPipelineConfig, options?: { pythonPath?: string; scriptPath?: string }) {
  ...
  async indexFile(filePath: string, docId?: string): Promise<ExtractedDocument> {
    const json = await this._extractViaPython("extract", filePath, docId ? { "--doc-id": docId } : {});
    return this.processAndStore(json);
  }

  /** Add raw text as a single chunk. */
  async indexText(text: string, source?: string, docId?: string): Promise<ExtractedDocument> {
    const json = await this._extractViaPython(
      "extract-from-text",
      text.slice(0, 64_000),
      { ...{ "--source": source || "manual" }, ...(docId ? { "--doc-id": docId } : {}) }
    );
    return this.processAndStore(json);
  }

  /** Search the indexed documents. */
  async search(text: string, topK = 5): Promise<Array<{ chunk: RAGChunk; score: number }>> {
  ...
  private async _extractViaPython(command: string, arg: string, extraArgs?: Record<string, string>): Promise<ExtractedDocumentJSON> {
    ...
    try {
      const { stdout } = await execFileAsync(this.pythonPath, args, { timeout: 120_000 });
  ...
  private async processAndStore(json: ExtractedDocumentJSON): Promise<ExtractedDocument> {
