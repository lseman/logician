...
export class MemoryVectorStore implements IVectorStore {
  ...
  async searchByVector(queryVec: number[], topK = 5): Promise<SearchHit[]> {
    const scored: Array<{ chunk: RAGChunk; score: number }> = this.chunks.map((chunk) => ({
      chunk,
  ...
  async search(_query: string, _topK = 5): Promise<SearchHit[]> {
    throw new Error("MemoryVectorStore.search() requires embedding the query first. Use searchByVector() with a pre-computed vector.");
  }
