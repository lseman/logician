...
// status instead of regex-sniffing the assistant text (see builtin-hooks
// declaresStop), and the afterToolCall hook terminates the run cleanly.
	...
	parameters: {
		...
		properties: {
			status: {
				type: "string",
				enum: ["done", "blocked"],
				description: "done = all requested work complete; blocked = cannot proceed.",
			},
			summary: {
				type: "string",
				description:
