...
export type AgentEventBody =
	...
	// Emitted when the active model changes (cycleModel or setModel).
	| { type: "model_update"; model: string }
	// Emitted when the tool set changes (setTools).
...
// Fires after the provider responds (before tool dispatch). Read-only: lets
// hooks observe raw model output, token usage, and stop reason for logging /
// analytics without altering the turn.
...
export interface GetFollowUpMessagesContext {
	...
	/** Hard safety cap on outer-loop continuations (follow-up driven). */
...
export interface AgentLoopHooks {
	...
	beforeProviderPayload?: (
		...
		| undefined;
	// Read-only observer after each provider response. Return value ignored.
...
export interface AgentConfig {
	...
	guardsEnabled?: boolean; // duplicate + failure-loop guards (default OFF — match pi's trust-model approach)
	...
	continuationEnabled?: boolean; // default OFF — match pi's approach
	maxContinuations?: number; // cap per run (default 12)
	// Loop detection and recovery: injects diagnostic messages when the model
	// appears stuck. Default OFF — the injected messages often cause more loops.
	loopDetectionEnabled?: boolean; // default OFF
