...
def _todo_command(
    *,
    ...
    if normalized_command == "add":
        ...
        _set_items(updated)
        return {"status": "ok", "added": new_item, "todos": updated, "view": _render_markdown(updated)}
    ...
    if normalized_command == "mark":
        ...
        _set_items(updated)
        return {"status": "ok", "updated": found, "todos": updated, "view": _render_markdown(updated)}
...
def _coerce_legacy_todos(todos: list[dict[str, Any]] | str | None) -> list[dict[str, Any]] | dict[str, Any]:
    if todos is None:
        return _get_items()
...
def _normalize_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(items, start=1):
        ...
        try:
            ...
            item_id = index
        title = (
            str(item.get("title") or item.get("content") or f"Task {index}").strip()
        )
