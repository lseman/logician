...
from ..repo.db import load_repo_graph_db, write_repo_graph_db
from ..repo.registry import ensure_repo_artifacts
from .parser import TREE_SITTER_EXTENSIONS, parse_file_symbols_imports, tree_sitter_available

_GRAPH_SOURCE_EXTENSIONS = set(TREE_SITTER_EXTENSIONS)
...
def _parse_symbols_and_imports_tree_sitter(
    rel_path: str,
    path: Path,
    text: str,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    if not tree_sitter_available() or path.suffix.lower() not in TREE_SITTER_EXTENSIONS:
        return [], []

    try:
        source = text.encode("utf-8", errors="replace")
        symbols, imports = parse_file_symbols_imports(path, source)
...
_JS_NAMED_IMPORT_LINE_RE = re.compile(r"""^\s*import\s*\{([^}]+)\}\s*from\s*['\"]([^'\"]+)['\"]""")
_JS_NAMED_EXPORT_LINE_RE = re.compile(
    r"""^\s*export\s*\{([^}]+)\}(?:\s*from\s*['\"]([^'\"]+)['\"])?"""
)
_JS_CALL_RE = re.compile(r"""\b(?:[A-Za-z_][A-Za-z0-9_]*\.)*([A-Za-z_][A-Za-z0-9_]*)\s*\(""")
...
_RUST_IMPL_RE = re.compile(r"""^\s*impl(?:<[^>]+>\s*)?\s+([A-Za-z_][A-Za-z0-9_:<>]*)\s*\{?""")
_RUST_TRAIT_DEF_RE = re.compile(r"""^\s*(?:pub\s+)?trait\s+([A-Za-z_][A-Za-z0-9_:<>]*)\s*\{?""")
_RUST_CALL_RE = re.compile(r"""\b(?:[A-Za-z_][A-Za-z0-9_]*::)*([A-Za-z_][A-Za-z0-9_]*)\s*\(""")
...
def _expand_brace_glob(glob_pattern: str) -> list[str]:
    ...
    return [f"{prefix}{part.strip()}{suffix}" for part in match.group(1).split(",") if part.strip()]


def query_repo_context(
    repo: dict[str, Any],
    *,
    query: str,
    limit: int = 8,
    ...
    terms = [
        term.lower()
        for term in re.findall(r"[A-Za-z_][A-Za-z0-9_./-]{1,}", str(query or ""))
        if len(term) >= 2
    ]

    def fuzzy_score(target: str) -> float:
        target_l = target.lower()
        if not query_l or not target_l:
            return 0.0
        if query_l in target_l or target_l in query_l:
            return 1.0
        return float(SequenceMatcher(None, query_l, target_l).ratio())

    def term_fuzzy_score(target: str) -> float:
        target_l = target.lower()
        if not target_l:
            return 0.0
        scores = [SequenceMatcher(None, term, target_l).ratio() for term in terms]
        return max(scores) if scores else 0.0

    if not terms:
        return {"candidate_paths": [], "matched_files": [], "matched_symbols": []}

    file_scores: dict[str, int] = {}
    symbol_rows: list[dict[str, Any]] = []

    def bump(rel_path: str, score: int) -> None:
        clean = str(rel_path or "").strip()
        if not clean:
            return
        file_scores[clean] = max(file_scores.get(clean, 0), int(score))
        ...
        if kind == "file" and rel_path:
            ...
            for term in terms:
                if term in lowered_path:
                    path_score = max(path_score, 90 if "/" in term or "." in term else 72)
            if not path_score and query_l:
                fuzzy = max(fuzzy_score(lowered_path), term_fuzzy_score(lowered_path))
                if fuzzy >= 0.60:
                    path_score = int(60 + fuzzy * 20)
            if path_score:
                bump(rel_path, path_score)
            continue

        if kind != "symbol" or not name:
            continue

        matching_terms = [term for term in terms if term in lowered_name]
        if not matching_terms:
            fuzzy = max(fuzzy_score(lowered_name), term_fuzzy_score(lowered_name))
            if fuzzy < 0.60:
                continue
            score = 84 if fuzzy < 0.80 else 92
        else:
            score = 96 if any(term == lowered_name for term in matching_terms) else 88
        if rel_path:
            bump(rel_path, score)
        symbol_rows.append(
            {
    ...
    seed_paths = [
        rel_path
        for rel_path, _score in sorted(file_scores.items(), key=lambda item: (-item[1], item[0]))
    ][: max(1, min(int(limit), 4))]
    related_files: list[dict[str, Any]] = []
    related_symbols: list[dict[str, Any]] = []
    if seed_paths:
        related = related_repo_context(repo, rel_paths=seed_paths, query=query, limit=max(2, limit))
        related_files = [
            {
        ...
        related_symbols = [
            ...
            for item in list(related.get("related_symbols") or [])
            if str(item.get("name") or "").strip()
        ]
        for row in related_files:
            bump(str(row.get("rel_path") or ""), int(row.get("score", 0) or 0))
    ...
    for row in sorted(
        symbol_rows + related_symbols,
        key=lambda item: (-int(item.get("score", 0) or 0), str(item.get("name") or "")),
    ):
        key = (
            str(row.get("name") or ""),
        ...
        if len(merged_symbols) >= max(1, int(limit)):
            break

    matched_files = [
        {"rel_path": rel_path, "score": score}
        for rel_path, score in sorted(file_scores.items(), key=lambda item: (-item[1], item[0]))
    ][: max(1, int(limit))]
    return {
        "candidate_paths": candidate_paths,
        "matched_files": matched_files,
        "matched_symbols": merged_symbols,
    }
...
def _matches_ignore_pattern(rel_path: Path, pattern: str) -> bool:
    ...
    if fnmatch.fnmatch(rel_posix, pattern) or fnmatch.fnmatch(rel_path.name, pattern):
        return True
    return any(part == pattern or fnmatch.fnmatch(part, pattern) for part in rel_path.parts)
...
def _is_excluded_relative_path(rel_path: Path, exclude_paths: list[str]) -> bool:
    ...
    for excluded in exclude_paths:
        if _matches_ignore_pattern(rel_path, excluded):
            return True
    return False
...
def _collect_matching_files(
    ...
    exclude_paths.extend(_load_gitignore_patterns(root))
    exclude_paths.extend(_parse_exclude_paths(exclude))
...
def _rust_symbols_and_imports(
    ...
    for idx, line in enumerate(text.splitlines(), start=1):
        ...
        context_stack = [
            context for context in context_stack if int(context.get("depth", 0) or 0) <= brace_depth
        ]
    imports: list[dict[str, Any]] = []
...
def _extract_symbols_and_imports(
    ...
    symbols, imports = _parse_symbols_and_imports_tree_sitter(rel_path, path, text)
    if symbols or imports:
        return symbols, imports
...
def _resolve_import_target(
    module: str, source_rel_path: str, path: Path, file_index: set[str]
) -> str | None:
    language = _language_for_path(path)
    if language == "python":
...
def build_repo_graph(
    ...
    *,
    ...
    file_index = {str(path.relative_to(repo_path).as_posix()) for path in files}
    ...
    db_path = write_repo_graph_db(repo, records)
...
def load_repo_graph(repo: dict[str, Any]) -> dict[str, Any]:
    ...
    db_path = Path(str(artifacts.get("db_path") or "")).expanduser()
    if db_path.exists():
        return load_repo_graph_db(repo)
...
def related_repo_context(
    ...
    *,
    ...
    query_l = str(query or "").lower()
    query_terms = [
        term.lower() for term in re.findall(r"[A-Za-z_][A-Za-z0-9_]{2,}", str(query or ""))
    ]
    ...
    def term_fuzzy_score(target: str) -> float:
        ...
        if not target_l:
            return 0.0
        scores = [SequenceMatcher(None, term, target_l).ratio() for term in query_terms]
        return max(scores) if scores else 0.0
