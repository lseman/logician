...
from .graph import _collect_matching_files, build_repo_graph
...
def ingest_repo(
    ...
    *,
    ...
    resolved_vector_path = None
    resolved_embedding_model = None
    resolved_vector_backend = None
    deleted_chunks = 0
    if rag_enabled:
    ...
    if rag_enabled and ingest_status in {"ok", "partial"}:
        repo_updates.update(
            {
    ...
    return {
        ...
        "vector_path": str(resolved_vector_path) if resolved_vector_path is not None else None,
        "vector_backend": resolved_vector_backend,
        "embedding_model_name": resolved_embedding_model,
